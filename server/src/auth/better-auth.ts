...
export function resolveBetterAuthSecret(): string {
  const fromBetterAuth = process.env.BETTER_AUTH_SECRET?.trim();
  if (fromBetterAuth) return fromBetterAuth;

  const fromAgentJwt = process.env.PAPERCLIP_AGENT_JWT_SECRET?.trim();
  if (fromAgentJwt) return fromAgentJwt;

  throw new Error(
    "BETTER_AUTH_SECRET (or PAPERCLIP_AGENT_JWT_SECRET) must be set. " +
    "For local development, set BETTER_AUTH_SECRET to a non-empty local secret in your .env file.",
  );
}
