...
import { environmentLeases, environments } from "@penclipai/db";
import {
  ENVIRONMENT_DRIVERS,
...
const kubernetesEnvironmentEnsureQueue = new Map<string, Promise<unknown>>();

async function withKubernetesEnvironmentEnsureQueue<T>(
  companyId: string,
  fn: () => Promise<T>,
): Promise<T> {
  const previous = kubernetesEnvironmentEnsureQueue.get(companyId) ?? Promise.resolve();
  const current = previous.catch(() => undefined).then(fn);
  kubernetesEnvironmentEnsureQueue.set(companyId, current);
  try {
    return await current;
  } finally {
    if (kubernetesEnvironmentEnsureQueue.get(companyId) === current) {
      kubernetesEnvironmentEnsureQueue.delete(companyId);
    }
  }
}
...
export interface KubernetesEnvironmentConfigInput {
   ...
   */
  timeoutMs?: number;
  adapters?: import("@penclipai/shared").AdapterRegistryEntry[];
  [key: string]: unknown;
}
...
export function environmentService(db: Db) {
  return {
    ...
    ensureKubernetesEnvironment: async (
      ...
      const queueKey = typeof companyIdOrConfig === "string" ? companyIdOrConfig : "__instance__";
      return withKubernetesEnvironmentEnsureQueue(queueKey, async () => {
