...
import type { DeploymentExposure, DeploymentMode, HumanCompanyMembershipRole, PermissionKey } from "@penclipai/shared";
import {
  forbidden,
  conflict,
  notFound,
...
function buildOnboardingDiscoveryDiagnostics(input: {
  ...
  if (
    ...
    diagnostics.push({
      ...
      hint: `Run npx penclip allowed-hostname ${apiHost}`
    });
  }
      ...
      connectivity: {
        ...
        guidance:
          ...
          opts.deploymentExposure === "private"
            ? "If OpenClaw runs on another machine, ensure the Paperclip hostname is reachable and allowed via `npx penclip allowed-hostname <host>`."
