...
import { LOW_TRUST_REVIEW_PRESET } from "@penclipai/shared";
...
function registerModuleMocks() {
  vi.doMock("@penclipai/adapter-opencode-local/server", async () => {
    const actual = await vi.importActual<typeof import("@penclipai/adapter-opencode-local/server")>("@penclipai/adapter-opencode-local/server");
    return {
      ...actual,
...
describe.sequential("agent permission routes", () => {
  beforeEach(() => {
    ...
    vi.doUnmock("@penclipai/adapter-opencode-local/server");
    registerModuleMocks();
    vi.resetAllMocks();
