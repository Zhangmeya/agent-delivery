...
const normalizePathForAssertion = (value: unknown) => String(value).replaceAll("\\", "/");
...
describe("agent routes adapter validation", () => {
  ...
  it("isolates CODEX_HOME when updating a codex_local agent to set its own OPENAI_API_KEY", async () => {
    ...
    expect(normalizePathForAssertion(env.CODEX_HOME)).toContain(`/companies/company-1/agents/${agentId}/codex-home`);
  });
