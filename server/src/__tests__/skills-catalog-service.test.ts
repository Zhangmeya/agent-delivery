...
import type { CatalogSkill } from "@penclipai/shared";
...
describe("skills catalog service", () => {
  ...
  beforeEach(() => {
    ...
    mockRequireResolve.mockImplementation((specifier: string) => {
      if (specifier === "@penclipai/skills-catalog/package.json") {
        return "/published/node_modules/@penclipai/skills-catalog/package.json";
      }
      if (specifier === "@penclipai/skills-catalog/catalog.json") {
        return "/published/node_modules/@penclipai/skills-catalog/generated/catalog.json";
      }
  ...
  it("resolves the manifest and bundled skill files from the published package layout", async () => {
    ...
    expect(mockReadFileSync).toHaveBeenCalledWith(
      "/published/node_modules/@penclipai/skills-catalog/generated/catalog.json",
      "utf8",
    );
    expect(String(mockReadFile.mock.calls[0]?.[0]).replace(/\\/g, "/")).toMatch(
      /\/published\/node_modules\/@penclipai\/skills-catalog\/catalog\/bundled\/software-development\/published-skill\/SKILL\.md$/,
    );
    expect(mockRequireResolve).toHaveBeenCalledWith("@penclipai/skills-catalog/package.json");
