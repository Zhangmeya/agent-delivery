...
import { writePaperclipSkillSyncPreference } from "@penclipai/adapter-utils/server-utils";
import {
  agents,
  companies,
  companySkills,
  ...
  async function seedIssueWithRichAgentComment() {
    ...
    await db.insert(companySkills).values([
      {
        id: randomUUID(),
        companyId,
        key: "penclipai/paperclip-cn/paperclip",
        slug: "paperclip",
        name: "Paperclip",
        markdown: "# Paperclip",
        sourceType: "catalog",
    ...
    await db.insert(agents).values({
      ...
      adapterConfig: writePaperclipSkillSyncPreference(
        ...
        ["penclipai/paperclip-cn/paperclip", "octo/research/public-skill"],
      ),
      runtimeConfig: {
        heartbeat: {
          enabled: true,
