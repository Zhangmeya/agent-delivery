...
describe("resolveServerVersion", () => {
  ...
  it("skips git metadata probing for packaged installs under node_modules", () => {
    ...
    expect(
      resolveServerVersion({
        ...
        debugLog,
        packageRoot: "/tmp/npm/_npx/example/node_modules/@penclipai/server",
      }),
  ...
  it("uses git metadata for source checkouts whose path contains node_modules", () => {
    ...
    const sourceCheckoutRoot = path.join(
    ...
    expect(
      resolveServerVersion({
        ...
        debugLog,
        gitDescribeCommand,
        packageRoot: path.join(sourceCheckoutRoot, "server"),
        pathExists: (candidate) => candidate === path.join(sourceCheckoutRoot, ".git"),
