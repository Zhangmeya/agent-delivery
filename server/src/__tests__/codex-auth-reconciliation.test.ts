...
describe("reconcileCodexLocalManagedHomesOnStartup", () => {
  ...
  async function expectSeededFromSharedAuth(agentAuth: string): Promise<void> {
    if (process.platform === "win32") {
      await expect(fs.readFile(agentAuth, "utf8")).resolves.toBe(
  ...
  it("seeds a stranded managed home and is a no-op on the next boot", async () => {
    ...
    await expectSeededFromSharedAuth(agentAuth);
