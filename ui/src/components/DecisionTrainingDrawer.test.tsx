...
const mockTranslation = vi.hoisted(() => ({
  t: vi.fn((key: string, options?: Record<string, unknown>) => {
    ...
    const template =
      count !== null && count !== 1 && options?.defaultValue_other
        ? String(options.defaultValue_other)
...
vi.mock("../api/decisionTraining", async (importOriginal) => {
  const original =
    await importOriginal<typeof import("../api/decisionTraining")>();
...
vi.mock("react-i18next", async (importOriginal) => {
  ...
  return {
    ...actual,
    useTranslation: () => ({ t: mockTranslation.t }),
  };
});
...
function findClickable(label: string): HTMLElement | null {
  return [...document.body.querySelectorAll("button, a")].find((el) =>
...
function buildSnapshot(): DecisionTrainingExample["snapshot"] {
  return {
    ...
    cutoff: {
      ...
      lastCommentId: "comment-abcdef12",
      commentCount: 3,
    },
    issue: {},
    comments: [{}, {}, {}],
...
function buildExample(
  overrides: Partial<DecisionTrainingExample> = {},
): DecisionTrainingExample {
  return {
    id: "example-1",
    companyId: "c1",
...
describe("DecisionTrainingDrawer — create state", () => {
  it("previews the frozen snapshot and saves an example", async () => {
    ...
    expect(bodyText()).toContain("Approve the migration plan?");
    expect(mockTranslation.t).toHaveBeenCalledWith(
      "decisionTraining.drawer.title.create",
      expect.objectContaining({ defaultValue: "Train this decision" }),
    );
    expect(mockTranslation.t).toHaveBeenCalledWith(
      "decisionTraining.drawer.outcome.accepted",
      expect.objectContaining({ defaultValue: "accepted" }),
    );
  ...
  it("refuses to train a decision with no issue anchor", () => {
    const item = buildItem({
      subject: { ...buildItem().subject, metadata: {} },
      relatedIssue: null,
    });
    render(
      <DecisionTrainingDrawer
...
describe("DecisionTrainingDrawer — saved state", () => {
  it("shows provenance and a read-only snapshot, and round-trips notes edits", async () => {
    ...
    mockApi.updateNotes.mockResolvedValue(
      buildExample({
        notes: "Revised reasoning.",
    ...
    expect(bodyText()).toContain(
      "I accepted because the plan covered rollback.",
    );
    expect(bodyText()).toContain("0123456789");
    ...
    expect(mockApi.updateNotes).toHaveBeenCalledWith(
      "example-1",
      "Revised reasoning.",
    );
  });
  ...
  it("deletes after confirmation", async () => {
    ...
    await waitFor(() => bodyText().includes("Frozen state"));
    await act(() => {
      findButton("Delete")?.dispatchEvent(
