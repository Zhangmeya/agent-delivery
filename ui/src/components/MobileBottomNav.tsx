...
export function MobileBottomNav({ visible }: MobileBottomNavProps) {
  ...
  const items = useMemo<MobileNavItem[]>(
    () => [
      { type: "link", to: "/dashboard", label: t("Home", { defaultValue: "Home" }), icon: House },
      { type: "link", to: "/issues", label: t("Issues", { defaultValue: "Issues" }), icon: CircleDot },
      { type: "action", label: t("Create", { defaultValue: "Create" }), icon: SquarePen, onClick: () => openNewIssue() },
      { type: "link", to: "/agents/all", label: t("Agents", { defaultValue: "Agents" }), icon: Users },
      {
        type: "link",
        to: "/inbox",
  ...
  return (
    <nav
      ...
      aria-label={t("Mobile navigation", { defaultValue: "Mobile navigation" })}
    >
