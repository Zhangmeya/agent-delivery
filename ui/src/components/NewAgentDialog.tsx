...
export function NewAgentDialog() {
  ...
  function handleAskCeo() {
    ...
    openNewIssue({
      ...
      title: t("newAgent.defaultIssueTitle", { defaultValue: "Create a new agent" }),
      description: t("newAgent.defaultIssueDescription", { defaultValue: "(type in what kind of agent you want here)" }),
    });
  }
  ...
  const createAgentInviteMutation = useMutation({
    ...
    onSuccess: async (invite) => {
      ...
      setLatestAgentPromptCopied(false);
      setMode("prompt");
      const copied = await copyText(prompt, t("newAgent.copyPromptManuallyBelow", {
        defaultValue: "Copy the agent onboarding prompt manually from the field below.",
      }));

      await queryClient.invalidateQueries({ queryKey: inviteHistoryQueryKey });
      pushToast({
        title: t("newAgent.agentInviteCreated", { defaultValue: "Agent invite created" }),
        body: copied
          ? t("newAgent.promptReadyCopied", { defaultValue: "Agent onboarding prompt ready below and copied to clipboard." })
          : t("newAgent.promptReady", { defaultValue: "Agent onboarding prompt ready below." }),
        tone: "success",
      });
    },
    onError: (error) => {
      pushToast({
        title: t("newAgent.failedToCreateAgentInvite", { defaultValue: "Failed to create agent invite" }),
    ...
    >
      ...
      >
        ...
        <div className="flex items-center justify-between px-4 py-2.5 border-b border-border">
          <span className="text-sm text-muted-foreground">{t("newAgent.title", { defaultValue: "Add a new agent" })}</span>
          <Button
            variant="ghost"
            size="icon-xs"
          ...
          {mode === "choices" ? (
            <>
              ...
              <div className="text-center space-y-3">
                ...
                <p className="text-sm text-muted-foreground">
                  {t("newAgent.choiceIntro", {
                    defaultValue: "Ask a leader to propose the hire, configure a runtime yourself, or send an onboarding prompt to an external agent.",
                  })}
                </p>
              </div>
              ...
              <Button className="w-full" size="lg" onClick={handleAskCeo}>
                ...
                {t("newAgent.askCeo", { defaultValue: "Ask the CEO to create a new agent" })}
              </Button>

              <div className="grid gap-2">
                <Button variant="outline" className="w-full" onClick={handleAdvancedConfig}>
                  <Settings2 className="h-4 w-4 mr-2" />
                  {t("newAgent.configureRuntimeManually", { defaultValue: "Configure a runtime manually" })}
                </Button>
                <div className="space-y-1">
                  <Button variant="outline" className="w-full" onClick={handleInviteExternalAgent}>
                    <MailPlus className="h-4 w-4 mr-2" />
                    {t("newAgent.inviteExternalAgent", { defaultValue: "Invite an external agent" })}
                  </Button>
                  <p className="text-xs text-muted-foreground text-center">
                    {t("newAgent.externalAgentExamples", {
            ...
            <>
              <div className="space-y-2">
                ...
                >
                  ...
                  {t("newAgent.back", { defaultValue: "Back" })}
                </button>
                <p className="text-sm text-muted-foreground">
                  {t("newAgent.runtimeChoiceHint", {
                  ...
                  >
                    {opt.recommended && (
                      <span className="absolute -top-1.5 right-1.5 bg-green-500 text-white text-(length:--text-nano) font-semibold px-1.5 py-0.5 rounded-full leading-none">
                        {t("newAgent.recommended", { defaultValue: "Recommended" })}
                      </span>
                    )}
            ...
            <div className="space-y-5">
              <div className="space-y-2">
                ...
                <div className="space-y-1">
                  <h2 className="text-sm font-semibold">{t("newAgent.inviteExternalAgent", { defaultValue: "Invite an external agent" })}</h2>
                  <p className="text-sm text-muted-foreground">
                    {t("newAgent.invitePromptDescription", {
                      defaultValue: "Generate a one-time onboarding prompt that any compatible agent can use to request access, wait for approval, and claim its Paperclip API key.",
                    })}
                  </p>
                </div>
              </div>
              ...
              <label className="block space-y-2">
                <span className="text-sm font-medium">{t("newAgent.optionalAgentMessage", { defaultValue: "Optional message for the agent" })}</span>
                <Textarea
                  value={agentMessage}
                  onChange={(event) => setAgentMessage(event.target.value)}
                  className="min-h-24 resize-y"
                  placeholder={t("newAgent.optionalAgentMessagePlaceholder", {
                    defaultValue: "Add onboarding context, expected role, or first instructions.",
                  })}
                  maxLength={4000}
                />
              </label>
              ...
              <div className="rounded-lg border border-border px-4 py-3 text-sm text-muted-foreground">
                {t("newAgent.agentInviteApprovalHint", {
                  defaultValue: "Agent invites create a join request first. A company admin still approves the request before the agent can claim its API key.",
                })}
              </div>
              ...
              <div>
                ...
                >
                  {createAgentInviteMutation.isPending
                    ? t("newAgent.generating", { defaultValue: "Generating…" })
                    : t("newAgent.generateOnboardingPrompt", { defaultValue: "Generate onboarding prompt" })}
                </Button>
              </div>
            </div>
          ) : (
            <div className="space-y-5">
              <div className="space-y-2">
                ...
                <div className="space-y-1">
                  <div className="flex items-center justify-between gap-3">
                    <h2 className="text-sm font-semibold">{t("newAgent.agentOnboardingPrompt", { defaultValue: "Agent onboarding prompt" })}</h2>
                    {latestAgentPromptCopied ? (
                  ...
                  <p className="text-sm text-muted-foreground">
                    {t("newAgent.sendPromptHint", {
                      defaultValue: "Send this prompt to the external agent that should join this company.",
                    })}
                  </p>
                </div>
              </div>
              ...
              <Button
                ...
                onClick={async () => {
                  ...
                  const copied = await copyText(latestAgentPrompt, t("newAgent.copyPromptManuallyAbove", {
                    defaultValue: "Copy the agent onboarding prompt manually from the field above.",
                  }));
                  setLatestAgentPromptCopied(copied);
                }}
              >
                {latestAgentPromptCopied
                  ? t("newAgent.copiedPrompt", { defaultValue: "Copied prompt" })
                  : t("newAgent.copyPrompt", { defaultValue: "Copy prompt" })}
              </Button>
            </div>
          )}
        </div>
