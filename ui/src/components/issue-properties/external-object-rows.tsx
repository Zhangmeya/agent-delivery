...
export function ExternalObjectRows({
  ...
  if (externalObjectsError) {
    return (
      <PropertyRow label={t("issueProperties.externalObjects", { defaultValue: "External objects" })}>
        <span className="text-xs text-muted-foreground">
          {t("issueProperties.externalObjectsLoadFailed", { defaultValue: "Couldn't load external objects." })}
          {onRetryExternalObjects ? (
            <>
              {" "}
              <button
  ...
  return (
    <>
      ...
      {expanded || hiddenExternalObjectCount > 0 ? (
        <PropertyRow label={t("issueProperties.references", { defaultValue: "References" })}>
