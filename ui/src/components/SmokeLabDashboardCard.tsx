        ...
        <div className="min-w-0">
          <div className="flex items-center gap-2">
            ...
            <p className="truncate text-sm font-semibold text-foreground">
              {t("apps.smokeLab.title", { defaultValue: "Integration smoke" })}
            </p>
          </div>
          <p className="mt-0.5 truncate text-xs text-muted-foreground">
            {t(`apps.smokeLab.health.${health}`, {
              defaultValue: {
          ...
          <p className="mt-0.5 truncate text-(length:--text-micro) text-muted-foreground/80">
            {latestRun
              ? t("apps.smokeLab.lastRun", {
                  defaultValue: "Last run {{time}}",
                  time: formatTime(latestRun.startedAt, i18n.resolvedLanguage ?? i18n.language),
                })
              : t("apps.smokeLab.runFromTab", { defaultValue: "Run one from the Smoke Lab tab" })}
          </p>
        </div>
      </div>
