...
import type { Agent, AcceptedPlanDecompositionSummary } from "@penclipai/shared";
      ...
      <div className="flex items-center justify-between gap-2">
        ...
        <span className="text-[11px] text-muted-foreground/80">
          {items.length === 1
            ...
            : t("issuePlanDecomposition.revisionCount_plural", {
                count: items.length,
                defaultValue: "{{count}} accepted plan revisions",
              })}
        </span>
      </div>
      ...
      <ul className="space-y-3">
        {items.map((record) => {
          ...
          const ownerName = record.ownerAgentId
            ? agentMap?.get(record.ownerAgentId)?.name ?? t("issuePlanDecomposition.fallbackAgent", { defaultValue: "agent" })
            : null;
          const revisionLabel =
            record.acceptedPlanRevisionNumber != null
              ? t("issuePlanDecomposition.revisionNumber", {
                  revision: record.acceptedPlanRevisionNumber,
                  defaultValue: "revision {{revision}}",
                })
              : t("issuePlanDecomposition.revisionId", {
                  revisionId: record.acceptedPlanRevisionId.slice(0, 8),
                  defaultValue: "revision {{revisionId}}",
                });
          const completedAt =
            ...
            >
              <div className="flex flex-wrap items-center gap-2">
                ...
                <span className="text-xs text-muted-foreground">
                  {t("issuePlanDecomposition.planRevision", {
                ...
                <span className="inline-flex items-center gap-1 text-xs text-foreground">
                  ...
                  {requested === 1
                    ? t("issuePlanDecomposition.childIssueCreated", {
                        created,
                        requested,
                        defaultValue: "{{created}} of {{requested}} child issue created",
                      })
                    : t("issuePlanDecomposition.childIssuesCreated", {
                        created,
                        requested,
                        defaultValue: "{{created}} of {{requested}} child issues created",
                      })}
                </span>
                {record.status === "completed" && requested > 0 ? (
                  ...
                  >
                    ...
                    {t("issuePlanDecomposition.idempotentClaim", { defaultValue: "Idempotent claim" })}
                  </span>
                ) : null}
              </div>
              ...
              <div className="mt-1 flex flex-wrap gap-x-3 gap-y-0.5 text-[11px] text-muted-foreground">
                ...
                {startedAt ? (
                  <span title={formatDateTime(startedAt)}>
                    {t("issuePlanDecomposition.started", {
                ...
                {completedAt ? (
                  <span title={formatDateTime(completedAt)}>
                    {t("issuePlanDecomposition.completedAt", {
                  ...
                  <span title={formatDateTime(updatedAt)}>
                    {t("issuePlanDecomposition.updated", {
                ...
                {issueIdentifier ? (
                  ...
                  >
                    {t("issuePlanDecomposition.planDocument", { defaultValue: "Plan document" })}
                  </Link>
                ) : null}
              </div>
