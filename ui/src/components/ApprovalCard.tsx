...
export function ApprovalCard({
  ...
  const kindLabel = t(typeLabel[approval.type] ?? approval.type, {
    defaultValue: typeLabel[approval.type] ?? approval.type,
  });
      ...
      <div className="flex items-start justify-between gap-4">
              ...
              <div className="flex flex-wrap items-center gap-2">
                ...
                {requesterAgent ? (
                  <div className="inline-flex min-w-0 items-center gap-1.5 text-xs text-muted-foreground">
                    <span>{t("Requested by", { defaultValue: "Requested by" })}</span>
              ...
              <div className="space-y-1">
                ...
                <p className="text-xs leading-5 text-muted-foreground">
                  {t("approval.requestCreated", {
                    defaultValue: "Approval request created {{time}}",
                    time: timeAgo(approval.createdAt),
                  })}
                </p>
              </div>
            </div>
        ...
        <div className="shrink-0">
          <div className="inline-flex items-center gap-1.5 rounded-full border border-border/70 bg-background/80 px-2.5 py-1 text-xs text-muted-foreground">
            ...
            <span>{translateStatusLabel(t, approval.status)}</span>
          </div>
        </div>
      </div>
              ...
              <>
                ...
                >
                  {pendingAction === "approve"
                    ? t("approval.approving", { defaultValue: "Approving..." })
                ...
                >
                  {pendingAction === "reject"
                    ? t("approval.rejecting", { defaultValue: "Rejecting..." })
