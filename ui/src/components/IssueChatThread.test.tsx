...
vi.mock("react-i18next", async (importOriginal) => {
  ...
  const translations: Record<string, string> = {
    ...
    "issueChat.noAssigneeSelectedDescription": "This comment will be posted without a responsible, so no agent will be woken to act on it. Go back to pick a responsible, or send anyway.",
    "issueChat.noAssigneeSelectedTitle": "No responsible selected",
    "status.blocked": "已阻塞",
    "status.done": "已完成",
...
describe("IssueChatThread", () => {
  ...
  it("renders deleted comments as tombstones without the original body", () => {
    ...
    expect(container.textContent).toContain("Me deleted this comment");
  ...
  it("localizes timeline status change values", async () => {
    const root = createRoot(container);
