...
export function RoutineTriggerCard({
  ...
  return (
    <form
      aria-label={t("routineDetail.triggerAriaLabel", { defaultValue: "Trigger: {{label}}", label: trigger.label ?? trigger.kind })}
        ...
        <div className="flex shrink-0 items-center gap-2">
          ...
          <span className="text-xs text-muted-foreground">
            {trigger.kind === "schedule" && trigger.nextRunAt
              ? t("routineDetail.nextAt", { defaultValue: "Next: {{time}}", time: new Date(trigger.nextRunAt).toLocaleString() })
