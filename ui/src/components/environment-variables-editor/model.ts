import type { CompanySecret, EnvBinding, SecretVersionSelector, UserSecretDefinition } from "@penclipai/shared";
...
export function validateName(
  ...
  if (!ENV_NAME_RE.test(trimmed)) {
    return {
      ...
      messageKey: "envVarEditor.invalidName",
    };
  }
  if (duplicateNames.has(trimmed)) {
    return { level: "error", message: "Duplicate name", messageKey: "envVarEditor.duplicateName" };
  }
  for (const prefix of reservedPrefixes) {
    if (prefix && trimmed.startsWith(prefix)) {
      return {
        level: "warn",
        message: "Reserved prefix - provided automatically and may be overridden",
        messageKey: "envVarEditor.reservedPrefix",
      };
    }
  }
  return null;
...
export function computeRowHealth(row: EnvRow, secrets: readonly CompanySecret[]): SecretHealth | null {
  ...
  if (!secret) {
    return {
      ...
      messageKey: "envVarEditor.secretMissingHealth",
    };
  }
  if (secret.status !== "active") {
    return {
      ...
      messageKey: "envVarEditor.secretDisabledHealth",
    };
  }
  return null;
}
