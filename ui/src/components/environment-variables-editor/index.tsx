...
import type { CompanySecret, EnvBinding, UserSecretDefinition } from "@penclipai/shared";
...
export const EnvironmentVariablesEditor = forwardRef<EnvironmentVariablesEditorHandle, EnvironmentVariablesEditorProps>(function EnvironmentVariablesEditor({
  ...
  function bulkImport(text: string, targetRowId: string): boolean {
    ...
    updateDraft(working);
    toast?.pushToast({
      title: t("envVarEditor.importedVariables", {
        defaultValue: "Imported {{count}} variable(s)",
        count: pairs.length,
      }),
      tone: "success",
    });
  ...
  const hint = footerHint === undefined
    ? t("agentConfig.envVarEditorHelp", { defaultValue: DEFAULT_HINT })
    : footerHint;
    ...
    <TooltipProvider>
      ...
      {attentionCount > 1 ? (
        <p className="inline-flex items-center gap-1.5 text-(length:--text-micro) font-medium text-amber-700 dark:text-amber-400">
          ...
          {t("agentConfig.secretBindingsNeedAttention", {
            defaultValue: "{{count}} secret binding(s) need attention:",
      ...
      {hasRows ? (
        ...
        <p className="text-sm text-muted-foreground">
          {t("envVarEditor.empty", { defaultValue: "No environment variables" })}
        </p>
      )}
      ...
      <div className="flex flex-wrap items-center justify-between gap-2 pt-1">
        ...
        >
          ...
          {t("envVarEditor.addVariable", { defaultValue: "Add variable" })}
        </button>
        ...
        {quickBind.length > 0 && !disabled ? (
          <div className="flex flex-wrap items-center gap-1.5">
            <span className="inline-flex items-center gap-1 text-(length:--text-micro) text-muted-foreground/70">
              ...
              {t("envVarEditor.recentlyUsed", { defaultValue: "Recently used:" })}
            </span>
        ...
        >
          <div className="min-w-0 space-y-1">
            <div className="flex min-w-0 items-center gap-2 text-sm font-medium">
              ...
              <span>{t("common.unsavedChanges", { defaultValue: "Unsaved changes" })}</span>
            </div>
            {changeSummaryText ? (
              <p className="truncate text-xs text-amber-900/80 dark:text-amber-100/80">
                {changeSummaryText}
              </p>
            ) : null}
          </div>
          <div className="flex items-center gap-2">
            ...
            >
              ...
              {t("envVarEditor.revert", { defaultValue: "Revert" })}
            </button>
            <button
              type="button"
              onClick={saveDraft}
