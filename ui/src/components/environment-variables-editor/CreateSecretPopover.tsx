...
export function SecretPopoverForm({
  ...
  const nameError = (() => {
    if (!trimmedName) {
      return touched
        ? t("envVarEditor.secretNameRequired", { defaultValue: "Name is required" })
        : null;
    }
    if (!SECRET_NAME_RE.test(trimmedName)) {
      return t("envVarEditor.secretNamePattern", { defaultValue: "Use lowercase letters, digits and _" });
    }
    if (existingSecretNames?.some((existing) => existing.toLowerCase() === trimmedName)) {
      return t("envVarEditor.secretNameExists", { defaultValue: "A secret with this name already exists" });
    }
    return null;
  })();
  const valueError = value.length === 0
    ? (touched ? t("envVarEditor.secretValueRequired", { defaultValue: "Value is required" }) : null)
    : null;
    ...
    try {
      ...
      setError(
        submitError instanceof Error
          ? submitError.message
          : t("agentConfig.failedToCreateSecret", { defaultValue: "Failed to create secret" }),
      );
      setSubmitting(false);
    }
  }
  ...
  const ctaLabel = mode === "create"
    ...
    : t("envVarEditor.storeAndBind", { defaultValue: "Store & bind" });
  const heading = mode === "create"
    ? t("secretBindingPicker.createSecret", { defaultValue: "Create secret" })
    : t("envVarEditor.storeValueAsSecret", { defaultValue: "Store value as secret" });
    ...
    <div className="w-72 space-y-3">
        ...
        {mode === "store" ? (
          <PopoverDescription className="text-[11px] text-muted-foreground">
            ...
            <span className="font-mono">
              {initialName || t("envVarEditor.thisVariable", { defaultValue: "this variable" })}
            </span>{" "}
            {t("envVarEditor.storeAsSecretDescriptionSuffix", { defaultValue: "to it." })}
          </PopoverDescription>
        ) : null}
      </div>
      ...
      <label className="block space-y-1">
        ...
        <input
          ...
          aria-label={t("envVarEditor.secretNameAria", { defaultValue: "Secret name" })}
      ...
      <label className="block space-y-1">
        ...
        <div className="relative">
          <input
            ...
            aria-label={t("envVarEditor.secretValueAria", { defaultValue: "Secret value" })}
          ...
          <button
            ...
            aria-label={reveal
              ? t("jsonSchemaForm.hideSecret", { defaultValue: "Hide secret" })
              : t("jsonSchemaForm.showSecret", { defaultValue: "Show secret" })}
