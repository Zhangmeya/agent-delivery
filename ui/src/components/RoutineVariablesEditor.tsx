...
import { isValidRoutineDateString, syncRoutineVariablesWithTemplate, type RoutineVariable } from "@penclipai/shared";
  ...
  return (
    <Collapsible open={open} onOpenChange={setOpen} className="overflow-hidden rounded-lg border border-border/70">
      <CollapsibleTrigger className="flex w-full items-center justify-between px-3 py-2 text-left">
        <div>
          <p className="text-sm font-medium">{t("Variables", { defaultValue: "Variables" })}</p>
          <p className="text-xs text-muted-foreground">
            {t("routineVariables.detectedFromPlaceholders", {
              defaultValue: "Detected from `{{placeholder}}` placeholders in the routine title and instructions.",
              placeholder: "{{name}}",
            })}
          </p>
        </div>
          ...
          <div key={variable.name} className="p-4">
            <div className="mb-3 flex flex-wrap items-center gap-2">
              ...
              <span className="text-xs text-muted-foreground">
                {t("routineVariables.promptBeforeRun", {
                  defaultValue: "Prompt the user for this value before each manual run.",
                })}
              </span>
            </div>
            ...
            <div className="grid gap-3 md:grid-cols-2">
              <div className="space-y-1.5">
                <Label className="text-xs">{t("Label", { defaultValue: "Label" })}</Label>
                <Input
              ...
              <div className="space-y-1.5">
                <Label className="text-xs">{t("Type", { defaultValue: "Type" })}</Label>
                <Select
                  value={variable.type}
              ...
              <div className="space-y-1.5 md:col-span-2">
                <div className="flex items-center justify-between gap-3">
                  <Label className="text-xs">{t("routineVariables.defaultValue", { defaultValue: "Default value" })}</Label>
                ...
                {variable.type === "textarea" ? (
                  ...
                  >
                    ...
                    <SelectContent>
                      <SelectItem value="__unset__">{t("routineVariables.noDefault", { defaultValue: "No default" })}</SelectItem>
                      <SelectItem value="true">{t("True", { defaultValue: "True" })}</SelectItem>
                      <SelectItem value="false">{t("False", { defaultValue: "False" })}</SelectItem>
                    </SelectContent>
                  </Select>
                ) : variable.type === "select" ? (
                  <div className="grid gap-3 md:grid-cols-2">
                    <div className="space-y-1.5">
                      <Label className="text-xs">{t("routineVariables.options", { defaultValue: "Options" })}</Label>
                      <Input
                    ...
                    <div className="space-y-1.5">
                      <Label className="text-xs">{t("routineVariables.defaultOption", { defaultValue: "Default option" })}</Label>
                      <Select
                      ...
                      >
                        <SelectTrigger>
                          <SelectValue placeholder={t("routineVariables.noDefault", { defaultValue: "No default" })} />
                        </SelectTrigger>
                        <SelectContent>
                          <SelectItem value="__unset__">{t("routineVariables.noDefault", { defaultValue: "No default" })}</SelectItem>
                  ...
                  <Input
                    ...
                    placeholder={variable.type === "number" ? "42" : t("routineVariables.defaultValue", { defaultValue: "Default value" })}
                  />
                )}
              </div>
            </div>
          ...
          <div className="space-y-5 text-sm">
            <section className="space-y-2">
              ...
              <ul className="list-disc space-y-1 pl-5 text-muted-foreground">
                ...
                <li>{t("routineVariables.typeRule", { defaultValue: "Pick a type (text, textarea, number, boolean, select, date), default value, and whether it is required." })}</li>
                <li>{t("routineVariables.dateRule", { defaultValue: "Variable names ending in capital Date, such as startDate, are created as date variables by default." })}</li>
