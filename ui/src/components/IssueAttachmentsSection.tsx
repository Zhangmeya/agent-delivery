    ...
    <div className="flex shrink-0 items-center gap-1">
      ...
      <Button asChild variant="ghost" size="icon-sm" title={t("issueOutput.openInNewTab", { defaultValue: "Open in new tab" })}>
        <a
          ...
          target="_blank"
          rel="noreferrer"
          aria-label={t("issueAttachments.openAria", { defaultValue: "Open {{filename}}", filename })}
        >
          <ExternalLink className="h-4 w-4" />
        </a>
      </Button>
      <Button asChild variant="ghost" size="icon-sm" title={t("Download", { defaultValue: "Download" })}>
        <a
          href={attachmentDownloadPath(attachment)}
          aria-label={t("issueOutput.downloadAria", { defaultValue: "Download {{filename}}", filename })}
        >
      ...
      <div className="mt-3 rounded-md hover:bg-accent/10">
        {isLoading ? (
          <p className="px-1 py-2 text-xs text-muted-foreground">
            {t("issueAttachments.loadingPreview", { defaultValue: "Loading preview..." })}
          </p>
        ) : error ? (
          <p className="px-1 py-2 text-xs text-destructive">
            {t("issueAttachments.previewLoadFailed", { defaultValue: "Could not load markdown preview." })}
          </p>
        ) : (
          <FoldCurtain>
    ...
    >
      <div className="flex items-center justify-between gap-2">
        <div className="flex items-center gap-2">
          ...
          <h3 className="text-sm font-medium text-muted-foreground">{t("Attachments", { defaultValue: "Attachments" })}</h3>
              ...
              {confirmDeleteId === attachment.id ? (
                ...
                >
                  <p className="text-xs font-medium text-white">
                    {t("issueAttachments.deletePromptShort", { defaultValue: "Delete?" })}
                  </p>
      ...
      {confirmDeleteId && !imageAttachments.some((attachment) => attachment.id === confirmDeleteId) ? (
        <div className="flex items-center justify-between gap-3 rounded-md border border-destructive/20 bg-destructive/5 px-4 py-3">
          <p className="text-sm font-medium text-destructive">
            {t("issueAttachments.deletePrompt", { defaultValue: "Delete this attachment? This cannot be undone." })}
          </p>
          <div className="flex shrink-0 items-center gap-2">
            ...
            <Button variant="destructive" size="sm" onClick={() => confirmDelete(confirmDeleteId)} disabled={deletePending}>
              {deletePending ? t("Deleting...", { defaultValue: "Deleting..." }) : t("Delete", { defaultValue: "Delete" })}
            </Button>
          </div>
        </div>
      ) : null}
