...
import type { ToolRiskLevel } from "@penclipai/shared";
...
function bindingRows(binding: ActionCardBinding, isStale: boolean, t: TFunction): BindingRow[] {
  const catalogValue = isStale && binding.previousCatalogSha256 ? (
    <span className="inline-flex flex-wrap items-center gap-1.5">
      <span
        className="text-muted-foreground line-through decoration-amber-500"
        title={t("actionCard.previousCatalogHash", { defaultValue: "Previous catalog hash" })}
      >
        {shortSha(binding.previousCatalogSha256)}
      </span>
      <span
        className="text-amber-600 dark:text-amber-400"
        title={t("actionCard.currentCatalogHash", { defaultValue: "Current catalog hash" })}
      >
  ...
  return [
    {
      label: t("actionCard.application", { defaultValue: "Application" }),
      value: (
        <span>
          {binding.application}
          <span className="ml-1.5 text-xs text-muted-foreground">
            {t("actionCard.manifestVersion", {
              defaultValue: "manifest v{{version}}",
              version: binding.manifestVersion,
            })}
          </span>
        </span>
      ),
    },
    { label: t("actionCard.connection", { defaultValue: "Connection" }), value: binding.connection, mono: true },
    { label: t("actionCard.catalog", { defaultValue: "Catalog" }), value: catalogValue, mono: !isStale },
    {
      label: t("actionCard.payload", { defaultValue: "Payload" }),
      value: (
        <span className="inline-flex items-center gap-1.5">
          ...
          <span className="font-sans text-(length:--text-micro) uppercase tracking-normal text-muted-foreground">
            {t("actionCard.signed", { defaultValue: "signed" })}
          </span>
        </span>
      ),
      mono: true,
...
export function ActionCard({
  ...
  const approveButton = (
    <Button
      ...
      title={isStale
        ? t("actionCard.approveDisabledTitle", {
          defaultValue: "Re-issue the request before approving — the catalog hash changed.",
        })
        : undefined}
    >
      {t("actionCard.approve", { defaultValue: "Approve" })}
    </Button>
  );
  const denyButton = (
    <Button size="sm" variant="outline" onClick={onDeny} className={mobile ? "w-full" : undefined}>
      {t("actionCard.deny", { defaultValue: "Deny" })}
    </Button>
  );
  const editButton = (
    <Button size="sm" variant="outline" onClick={onEditResign} className={mobile ? "w-full" : undefined}>
      <Pencil className="mr-1 h-3.5 w-3.5" />
      {t("actionCard.editAndResign", { defaultValue: "Edit & re-sign" })}
    </Button>
  );
    ...
    >
      <CardContent className="space-y-3 p-4">
        ...
        <div className="flex items-start gap-3">
          ...
          <div className="min-w-0 flex-1">
            <p className="text-sm text-foreground">
              ...
              {t("actionCard.requestedApproval", { defaultValue: "requested approval to call" })}
            </p>
        ...
        {isStale ? (
          <EnforcementBanner
            tone="warning"
            title={t("actionCard.staleTitle", {
              defaultValue: "Catalog changed since this request was signed.",
            })}
            body={t("actionCard.staleBody", {
              defaultValue: "The application's tool catalog hash no longer matches the one this approval was issued against. Approval is disabled — the agent must edit & re-sign to request again.",
            })}
          />
        ) : null}

        {/* Bindings table */}
        <BindingsTable rows={bindingRows(binding, isStale, t)} labelWidth={mobile ? 70 : 132} />

        {/* JSON input */}
        <div className="space-y-1">
          <p className="text-xs font-medium uppercase tracking-normal text-muted-foreground">
            {t("actionCard.input", { defaultValue: "Input" })}
          </p>
        ...
        <div className="space-y-1">
          <p className="text-xs font-medium uppercase tracking-normal text-muted-foreground">
            {t("actionCard.whyAsking", { defaultValue: "Why I'm asking" })}
          </p>
            ...
            {policyNumber != null ? (
              <>
                ...
                <span className="font-medium text-foreground">
                  {t("actionCard.policyNumber", {
                    defaultValue: "Policy #{{number}}",
                    number: policyNumber,
                  })}
                </span>{" "}
                {t("actionCard.policyRequiresApproval", { defaultValue: "requires approval here." })}
              </>
            ) : null}
          </p>
        </div>
      ...
      >
        <span className="flex items-center gap-1.5 text-xs text-muted-foreground">
          ...
          {expiresInLabel ?? t("actionCard.noExpiry", { defaultValue: "no expiry set" })}
        </span>
        {mobile ? (
