...
function localizedDenialCopy(t: TFunction, denial: SkillDenial) {
  return {
    title: t(denial.titleKey, {
      ...denial.titleValues,
      defaultValue: denial.title,
    }),
    remediation: denial.remediationKey
      ? t(denial.remediationKey, { defaultValue: denial.remediation })
      : denial.remediation,
  };
}
...
export function SkillPolicyDenialNotice({
  ...
  const copy = localizedDenialCopy(t, denial);
  const actions = onDismiss ? (
    <Button variant="ghost" size="sm" onClick={onDismiss}>
      {t("skillPolicyDenial.dismiss", { defaultValue: "Dismiss" })}
    </Button>
  ) : undefined;
