...
export function PipelineItemBodyDocument({
  ...
  const restoreMutation = useMutation({
    ...
    onSuccess: async () => {
      ...
      await invalidateAll();
      pushToast({ title: t("pipelineItemBody.revisionRestored", { defaultValue: "Revision restored" }), tone: "success" });
    },
    onError: () => pushToast({
      title: t("pipelineItemBody.restoreRevisionFailed", { defaultValue: "Could not restore the revision" }),
      tone: "error",
    }),
  });
  ...
  const handleSave = useCallback(async () => {
    try {
      ...
      if (error instanceof ApiError && error.status === 409) {
        ...
        pushToast({
          title: t("pipelineItemBody.bodyChangedElsewhere", { defaultValue: "Body changed elsewhere" }),
          body: t("pipelineItemBody.bodyChangedElsewhereDescription", {
            defaultValue: "This item body was updated by someone else. Reloaded the latest - re-apply your edit.",
          }),
          tone: "error",
        });
        return;
      }
      pushToast({ title: t("pipelineItemBody.saveBodyFailed", { defaultValue: "Could not save the body" }), tone: "error" });
    }
  }, [caseDocumentQuery, doc?.latestRevisionId, draftBody, pushToast, saveMutation, t]);
  ...
  const handleStartConversationFromAnchor = useCallback(
    async (anchor: PendingAnchor) => {
      ...
      try {
        ...
        setPendingStartAnchor(null);
        pushToast({
          title: t("pipelineItemBody.startConversationFailed", { defaultValue: "Could not start the conversation" }),
          tone: "error",
        });
      }
    },
  ...
  if (editing) {
          ...
          <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
            <div className="space-y-1">
              ...
              <p className="text-xs text-muted-foreground">
                {t("pipelineItemBody.historicalPreviewNotice", {
                  defaultValue: "Historical preview. New comments are disabled while previewing a historical revision. Restoring it creates a new latest revision and keeps history append-only.",
                })}
              </p>
            </div>
    ...
    bodyContent = (
      <EmptyState
        icon={FileText}
        message={t("pipelineItemBody.emptyMessage", { defaultValue: "No body yet. Capture the item's details here." })}
        action={t("pipelineItemBody.addBody", { defaultValue: "Add the item body" })}
        onAction={beginEdit}
      />
    );
  ...
  return (
    <section
      aria-label={t("pipelineItemBody.itemBody", { defaultValue: "Item body" })}
    ...
    >
      <DocumentFrameHeader
        ...
        documentLabel={t("pipelineItemBody.documentLabel", { defaultValue: "Item body document" })}
        folded={folded}
