...
const SCOPE_LABEL_KEYS: Record<CompanySearchScope, string> = {
  all: "searchPage.scope.all",
  issues: "searchPage.scope.issues",
  comments: "searchPage.scope.comments",
  documents: "searchPage.scope.documents",
  artifacts: "searchPage.scope.artifacts",
  agents: "searchPage.scope.agents",
...
const SUBGROUP_LABEL_KEYS: Record<SubGroupKey, string> = {
  ...
  comments: "searchPage.subgroup.comments",
  documents: "searchPage.subgroup.documents",
  artifacts: "searchPage.subgroup.artifacts",
  agents: "searchPage.subgroup.agents",
  projects: "searchPage.subgroup.projects",
};
...
function describeScope(t: TFunction, scope: CompanySearchScope) {
  if (scope === "all") return t("searchPage.scope.allScopes");
      ...
      <div className="border-b border-border px-4 py-3 sm:px-6">
        ...
        <div className="relative">
          ...
          <Input
            ...
            aria-label={t("searchPage.searchAria")}
        ...
        <div className="flex flex-wrap items-center justify-center gap-2">
          {scope !== "all" ? (
            <Button onClick={showAllScope} size="sm" variant="outline">
              {t("searchPage.searchAllScopes")}
            </Button>
          ) : null}
          <Button onClick={openNewIssue} size="sm" variant="default">
            <Plus className="mr-1.5 h-4 w-4" />
            {t("searchPage.createIssueFromQuery")}
          </Button>
