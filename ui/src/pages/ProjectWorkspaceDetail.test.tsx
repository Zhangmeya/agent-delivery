...
import type { Project, ProjectWorkspace } from "@penclipai/shared";
...
const mockProjectQueryState = vi.hoisted(() => ({
...
vi.mock("@tanstack/react-query", () => ({
  ...
  useQuery: () => mockProjectQueryState,
...
vi.mock("react-i18next", async (importOriginal) => {
  ...
  const translations: Record<string, string> = {
    "projectWorkspace.backToConfiguration": "Back to configuration",
    "projectWorkspace.configuration": "Configuration",
    "projectWorkspace.isPrimaryDescription": "This is the project’s primary codebase workspace.",
    "projectWorkspace.loadingPlugin": "Loading workspace plugin...",
...
describe("ProjectWorkspaceDetail plugin tabs", () => {
  ...
  beforeEach(() => {
    ...
    mockProjectQueryState.data = project();
    mockProjectQueryState.isLoading = false;
    mockProjectQueryState.error = null;
  ...
  async function render() {
    await act(async () => {
      ...
      root.render(<ProjectWorkspaceDetail />);
    });
    await act(async () => {
      await flush();
    });
