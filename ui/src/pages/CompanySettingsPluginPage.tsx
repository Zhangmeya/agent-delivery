...
export function CompanySettingsPluginPage() {
  ...
  if (!resolvedCompanyId) {
    ...
    return <div className="text-sm text-muted-foreground">{t("Select a company to view this page.")}</div>;
  }
  ...
  if (errorMessage) {
    return (
      <div className="rounded-md border border-destructive/30 bg-destructive/5 px-3 py-2 text-sm text-destructive">
        {t("Plugin extensions unavailable: {{message}}", { message: errorMessage })}
      </div>
    );
  }
  ...
  if (pageSlots.length > 1) {
    return (
      <div className="rounded-md border border-destructive/30 bg-destructive/5 px-3 py-2 text-sm text-destructive">
        {t("Multiple plugins declare the company settings route {{route}}. Disable one plugin or change its route.", { route: settingsRoutePath })}
      </div>
    );
  }
