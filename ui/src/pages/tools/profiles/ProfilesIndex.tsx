...
export function ProfilesIndex({
  ...
  const duplicate = useMutation({
    mutationFn: (profile: ToolProfileWithDetails) =>
      toolsApi.duplicateProfile(profile.id, {
        name: t("tools.profiles.dialog.copyNameParenthetical", { defaultValue: "{{name}} (copy)", name: profile.name }),
      }),
    onSuccess: () => {
      pushToast({
    ...
    onError: (error: unknown) =>
      pushToast({ title: t("tools.profiles.toast.duplicateFailed", { defaultValue: "Could not duplicate" }), body: errorBody(error), tone: "error" }),
  });
  ...
  const archive = useMutation({
    ...
    onSuccess: () => {
      pushToast({ title: t("tools.profiles.toast.archived", { defaultValue: "Profile archived" }), tone: "success" });
      invalidate();
    },
    onError: (error: unknown) => pushToast({ title: t("tools.profiles.toast.archiveFailed", { defaultValue: "Could not archive" }), body: errorBody(error), tone: "error" }),
  });
  ...
  const restore = useMutation({
    ...
    onSuccess: () => {
      pushToast({ title: t("tools.profiles.toast.restored", { defaultValue: "Profile restored" }), tone: "success" });
      invalidate();
    },
    onError: (error: unknown) => pushToast({ title: t("tools.profiles.toast.restoreFailed", { defaultValue: "Could not restore" }), body: errorBody(error), tone: "error" }),
  });
  ...
  const header = (
    <ToolsPageHeader
      ...
      description={t("tools.profiles.description", { defaultValue: "Decide which tools your agents can use. Build a profile once, then assign it to the agents that need it." })}
      actions={
        <>
          <Button variant="outline" onClick={() => setResolverOpen(true)}>
            <ShieldCheck className="mr-1.5 h-4 w-4" />
            {t("tools.profiles.checkAgentAccess", { defaultValue: "Check an agent's access" })}
          </Button>
    ...
    <Sheet open={resolverOpen} onOpenChange={setResolverOpen}>
      <SheetContent className="w-full gap-0 p-0 sm:max-w-xl">
        <SheetHeader className="border-b border-border">
          <SheetTitle>{t("tools.profiles.checkAgentAccess", { defaultValue: "Check an agent's access" })}</SheetTitle>
          <SheetDescription>
            {t("tools.profiles.checkAgentAccessDescription", { defaultValue: "See exactly which tools an agent can use right now, and which profile allows each one." })}
          </SheetDescription>
        </SheetHeader>
  ...
  if (profiles.isLoading) {
    return (
      <div className="space-y-5">
        {header}
        <LoadingState label={t("tools.profiles.loading", { defaultValue: "Loading profiles…" })} />
      </div>
    );
  }
  if (profiles.isError) {
    ...
    <div className="space-y-5">
      ...
      <div className="inline-flex rounded-md border border-border p-0.5">
        {(["active", "archived"] as const).map((key) => (
          ...
          >
            {key === "active"
              ? t("tools.profiles.status.active", { defaultValue: "Active" })
              : t("tools.profiles.status.archived", { defaultValue: "Archived" })}
          </button>
        ))}
      </div>
      ...
      {rows.length === 0 ? (
        statusFilter === "archived" ? (
          <div className="rounded-lg border border-dashed border-border px-4 py-5 text-sm text-muted-foreground">
            {t("tools.profiles.emptyArchived", { defaultValue: "No archived profiles." })}
          </div>
        ) : (
          ...
          <table className="w-full table-fixed text-sm">
            ...
            <thead>
              <tr className="border-b border-border bg-muted/40 text-left text-xs font-medium text-muted-foreground">
                <th className="px-3 py-2 font-medium">{t("tools.profiles.table.profile", { defaultValue: "Profile" })}</th>
                <th className="px-3 py-2 font-medium">{t("tools.profiles.table.allows", { defaultValue: "Allows" })}</th>
                <th className="px-3 py-2 font-medium">{t("tools.profiles.table.assignedTo", { defaultValue: "Assigned to" })}</th>
                <th className="px-3 py-2 font-medium">{t("tools.common.status", { defaultValue: "Status" })}</th>
                <th className="px-3 py-2 font-medium">{t("tools.common.updated", { defaultValue: "Updated" })}</th>
              ...
              {rows.map((profile) => {
                ...
                return (
                  ...
                  >
                      ...
                      <span className="inline-flex items-center gap-2">
                        ...
                        {(profile.newToolsPendingCount ?? 0) > 0 ? (
                          <Badge variant="outline" className="border-amber-500/50 bg-amber-500/10 text-amber-800 dark:text-amber-200">
                            {t("tools.profiles.table.newTools", { defaultValue: "{{count}} new", count: profile.newToolsPendingCount })}
                          </Badge>
                        ) : null}
                      </span>
                    </td>
                    <td className="px-3 py-1.5">
                      {assigned.unassigned ? (
                        <span className="text-muted-foreground">
                          {assigned.text}
                          <span className="ml-1 text-xs text-muted-foreground/70">— {t("tools.profiles.table.noAccessEffect", { defaultValue: "does not change access" })}</span>
                        </span>
                      ) : (
                    ...
                    <td className="px-3 py-1.5">
                      <span className="inline-flex items-center gap-2">
                        <Badge variant={statusVariant(profile.status)}>{t(`tools.profiles.status.${profile.status}`, { defaultValue: STATUS_LABEL[profile.status] })}</Badge>
                        {draft ? (
                          <button
                            type="button"
                            onClick={open}
                          ...
                          >
                            {t("tools.common.resume", { defaultValue: "Resume" })}
                          </button>
                        ) : null}
                      </span>
                    </td>
  ...
  return (
    <DropdownMenu>
      <DropdownMenuTrigger asChild>
        <button
          type="button"
          aria-label={t("tools.profiles.rowActions", { defaultValue: "Profile actions" })}
      ...
      <DropdownMenuContent align="end">
        <DropdownMenuItem onSelect={onEdit}>{t("tools.common.edit", { defaultValue: "Edit" })}</DropdownMenuItem>
        <DropdownMenuItem onSelect={onDuplicate}>{t("tools.common.duplicate", { defaultValue: "Duplicate" })}</DropdownMenuItem>
        {onRestore ? (
          ...
          <DropdownMenuItem onSelect={onArchive}>{t("tools.common.archive", { defaultValue: "Archive" })}</DropdownMenuItem>
        )}
        <DropdownMenuSeparator />
  ...
  return (
    <div className="rounded-lg border border-dashed border-border p-6">
      <div className="mb-4 max-w-2xl">
        <h3 className="text-base font-semibold text-foreground">{t("tools.profiles.empty.title", { defaultValue: "Create your first access profile" })}</h3>
        <p className="text-sm text-muted-foreground">
          {t("tools.profiles.empty.description", { defaultValue: "Pick a starting point. You can fine-tune exactly which tools it allows in the next step." })}
        </p>
      </div>
      <div className="grid gap-2 sm:grid-cols-2 lg:grid-cols-3">
        {TEMPLATES.map((template) => (
          ...
          >
            <span className="text-sm font-medium text-foreground">{t(TEMPLATE_TRANSLATIONS[template.key].titleKey, { defaultValue: template.title })}</span>
            <span className="text-xs text-muted-foreground">{t(TEMPLATE_TRANSLATIONS[template.key].descriptionKey, { defaultValue: template.description })}</span>
          </button>
        ))}
      </div>
    </div>
