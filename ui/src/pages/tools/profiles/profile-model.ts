  ...
  for (const tool of catalog) {
    ...
    if (!group) {
      const name =
        ...
        translateInstant("tools.profiles.model.unnamedTools", { defaultValue: "Tools" });
      group = {
        appKey,
...
export function appSelectionLabel(group: AppGroup, selection: AppSelection | undefined): string {
  ...
  if (state === "unchecked") {
    return translateInstant("tools.profiles.selection.none", { defaultValue: "None selected" });
  }
  if (selection?.kind === "all" || (selection?.kind === "all_except" && selection.excluded.length === 0)) {
    return translateInstant("tools.profiles.selection.all", {
      defaultValue: "All {{app}} tools ({{count}})",
      app: group.name,
      count: total,
    });
  }
  if (selection?.kind === "all_except") {
    ...
    return translateInstant("tools.profiles.selection.allExcept", {
      defaultValue: "All {{app}} except {{count}}",
      app: group.name,
      count: n,
    });
  }
  const n = selectedToolIds(group, selection).size;
  return translateInstant("tools.profiles.selection.some", {
    defaultValue: "{{selected}} of {{total}} {{app}} tools",
    selected: n,
    total,
    app: group.name,
  });
