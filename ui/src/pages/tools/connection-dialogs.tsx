  ...
  return (
    <Dialog open onOpenChange={(o) => !o && onClose()}>
      <DialogContent className="max-w-2xl">
        <DialogHeader>
          <DialogTitle>
            {t("tools.connections.catalog.title", {
              defaultValue: "Tool catalog — {{connection}}",
...
export function AddConnectionDialog({
  ...
  const create = useMutation({
    ...
    onError: (err) =>
      pushToast({
        title: t("tools.connections.toast.createFailed", { defaultValue: "Could not create connection" }),
  ...
  const probe = useMutation({
    ...
    onError: (err) =>
      pushToast({
        title: t("tools.connections.toast.probeFailed", { defaultValue: "Probe failed" }),
        body: err instanceof ApiError ? err.message : String(err),
        tone: "error",
      }),
  });
  ...
  const activate = useMutation({
    ...
    onSuccess: () => {
      ...
      pushToast({
        title: t("tools.connections.toast.activated", { defaultValue: "Connection activated" }),
        tone: "success",
      });
      onClose();
    },
    onError: (err) =>
      pushToast({
        title: t("tools.connections.toast.activationFailed", { defaultValue: "Activation failed" }),
        body: err instanceof ApiError ? err.message : String(err),
        tone: "error",
      }),
  });
  ...
  return (
    <Dialog open onOpenChange={(o) => !o && onClose()}>
      <DialogContent className="max-w-xl">
        <DialogHeader>
          <DialogTitle>{t("tools.connections.add.title", { defaultValue: "Add application" })}</DialogTitle>
          <DialogDescription>
            {t("tools.connections.add.description", {
              defaultValue:
                "Choose an existing application or create one as part of the same connection flow. Credentials stay as vault references and the connection is probed before activation.",
            })}
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="space-y-3">
          <div className="flex items-center gap-2 text-xs text-muted-foreground">
            <span className={step === 1 ? "font-medium text-foreground" : ""}>
              {t("tools.connections.add.steps.application", { defaultValue: "1 Application" })}
            </span>
            <span>/</span>
            <span className={step === 2 ? "font-medium text-foreground" : ""}>
              {t("tools.connections.add.steps.connection", { defaultValue: "2 Connection" })}
            </span>
          </div>
          ...
          {step === 1 && !locked ? (
            <>
              <div className="space-y-1.5">
                <Label>{t("tools.connections.add.application", { defaultValue: "Application" })}</Label>
                <Select value={applicationMode} onValueChange={(v) => setApplicationMode(v as "existing" | "new")}>
                  ...
                  <SelectContent>
                    <SelectItem value="existing">
                      {t("tools.connections.add.useExistingApplication", { defaultValue: "Use existing application" })}
                    </SelectItem>
                    <SelectItem value="new">
                      {t("tools.connections.add.createNewApplication", { defaultValue: "Create new application" })}
                    </SelectItem>
                  </SelectContent>
                </Select>
              </div>
              ...
              {applicationMode === "existing" ? (
                <div className="space-y-1.5">
                  <Label>{t("tools.connections.add.existingApplication", { defaultValue: "Existing application" })}</Label>
                  <Select value={applicationId} onValueChange={setApplicationId}>
                    <SelectTrigger>
                      <SelectValue
                        placeholder={t("tools.connections.add.selectApplication", {
                ...
                <div className="space-y-1.5">
                  <Label htmlFor="app-name">
                    {t("tools.connections.add.newApplicationName", { defaultValue: "New application name" })}
                  </Label>
                  <Input
                    id="app-name"
                    ...
                    placeholder={t("tools.connections.add.applicationNamePlaceholder", {
                      defaultValue: "e.g. GitHub Triage",
                    })}
                  />
                  <p className="text-xs text-muted-foreground">
                    {t("tools.connections.add.applicationTypeHint", {
                      defaultValue: "Application type is inferred from the transport you choose next.",
                    })}
                  </p>
                </div>
              )}
            ...
            <>
              {applicationMode === "new" ? (
                <div className="rounded-md border border-border bg-muted/30 px-3 py-2 text-xs text-muted-foreground">
                  {t("tools.connections.add.applicationWillBeCreated", {
                    defaultValue: "{{application}} will be created as {{type}}.",
              ...
              <div className="space-y-1.5">
                <Label htmlFor="conn-name">
                  {t("tools.connections.add.connectionName", { defaultValue: "Connection name" })}
                </Label>
                <Input
                  id="conn-name"
                  value={name}
                  ...
                  placeholder={t("tools.connections.add.connectionNamePlaceholder", {
                    defaultValue: "e.g. Production GitHub",
                  })}
                  disabled={locked}
                />
              </div>
              ...
              <div className="space-y-1.5">
                <Label>{t("tools.connections.add.transport", { defaultValue: "Transport" })}</Label>
                <Select
                  value={transport}
                ...
                >
                  ...
                  <SelectContent>
                    <SelectItem value="remote_http">
                      {t("tools.connections.add.transportRemoteHttp", {
                        defaultValue: "Remote HTTP (no local process)",
                      })}
                    </SelectItem>
                    <SelectItem value="local_stdio">
                      {t("tools.connections.add.transportLocalStdio", {
                        defaultValue: "Local stdio (approved template)",
                      })}
                    </SelectItem>
                  </SelectContent>
                </Select>
              ...
              {transport === "remote_http" ? (
                <div className="space-y-1.5">
                  <Label htmlFor="conn-url">
                    {t("tools.connections.add.endpointUrl", { defaultValue: "Endpoint URL" })}
                  </Label>
                  <Input
                    id="conn-url"
                    value={endpointUrl}
                ...
                <div className="space-y-1.5">
                  <Label>{t("tools.connections.add.commandTemplate", { defaultValue: "Command template" })}</Label>
                  <Select value={templateId} onValueChange={setTemplateId} disabled={locked}>
                    <SelectTrigger>
                      <SelectValue
                        placeholder={t("tools.connections.add.selectTemplate", {
                          defaultValue: "Select an approved template",
                        })}
                      />
                    </SelectTrigger>
                    <SelectContent>
                  ...
                  <p className="text-xs text-muted-foreground">
                    {t("tools.connections.add.commandTemplateHint", {
                      defaultValue: "Only board-approved command templates can run. Arbitrary commands are never accepted.",
                    })}
                  </p>
                </div>
              )}
              ...
              <div className="space-y-1.5">
                <Label>
                  {t("tools.connections.add.credentialReferences", { defaultValue: "Credential references" })}
                </Label>
                {creds.length > 0 ? (
                      ...
                      >
                        ...
                        {!locked ? (
                          <button
                            ...
                            aria-label={t("tools.connections.add.removeCredentialAria", {
                              defaultValue: "Remove credential reference for {{secret}}",
                              secret: secretName(c.secretId),
                            })}
                          >
                ...
                {!locked ? (
                  <>
                    <div className="flex items-end gap-2">
                      <div className="flex-1 space-y-1">
                        <Select value={pendingSecretId} onValueChange={setPendingSecretId}>
                          <SelectTrigger>
                            <SelectValue
                              placeholder={t("tools.connections.add.selectVaultSecret", {
                                defaultValue: "Select a vault secret",
                              })}
                            />
                          </SelectTrigger>
                          <SelectContent>
                      ...
                      <Input
                        ...
                        placeholder={t("tools.connections.add.headerPlaceholder", { defaultValue: "Header" })}
                        className="w-32"
                        aria-label={t("tools.connections.add.headerNameAria", { defaultValue: "Header name" })}
                      />
                      <Button type="button" size="sm" variant="outline" onClick={addCred} disabled={!pendingSecretId}>
                        {t("tools.common.add", { defaultValue: "Add" })}
                      </Button>
                    </div>
                    {pendingSecretId ? (
                    ...
                    <p className="text-xs text-muted-foreground">
                      {t("tools.connections.add.vaultReferenceHint", {
                        defaultValue:
                          "Free-text secrets are not accepted — pick a vault entry; Paperclip stores only the vault:// reference and resolves it at gateway use time.",
                      })}
                    </p>
                  </>
                ) : null}
          ...
          {locked ? (
            probe.isPending ? (
              <div className="rounded-md border border-border bg-muted/40 p-3">
                <LoadingState
                  label={t("tools.connections.add.probingConnection", { defaultValue: "Probing connection…" })}
                />
              </div>
            ) : probe.isError ? (
              ...
              <div className="rounded-md border border-border bg-muted/40 p-3">
                <div className="flex items-center gap-2 text-sm">
                  <span className="font-medium text-foreground">
                    {t("tools.connections.add.probeResult", { defaultValue: "Probe result" })}
                  </span>
                ...
                <div className="mt-2 grid grid-cols-3 gap-2">
                  <div>
                    ...
                    <p className="text-xs text-muted-foreground">
                      {t("tools.connections.add.toolsDiscovered", { defaultValue: "tools discovered" })}
                    </p>
                  </div>
                  <div>
                    <p className="text-lg font-semibold tabular-nums text-foreground">
                      {probeResult.latencyMs != null
                        ? t("tools.connections.add.latencyValue", {
                    ...
                    <p className="text-xs text-muted-foreground">
                      {t("tools.connections.add.probeLatency", { defaultValue: "probe latency" })}
                    </p>
                  </div>
                  <div>
                    <p className="text-lg font-semibold tabular-nums text-foreground">
                    ...
                    <p className="text-xs text-muted-foreground">
                      {t("tools.connections.add.quarantined", { defaultValue: "quarantined" })}
                    </p>
                  </div>
                </div>
                ...
                <p className="mt-2 text-(length:--text-micro) text-muted-foreground">
                  {t("tools.connections.add.probeLatencyHint", {
                    defaultValue:
                      "Probe latency is a single round-trip sample. Aggregate p95 latency across traffic is tracked on the Runtime tab once the connection is live.",
                  })}
                </p>
              </div>
            ) : null
          ...
          {step === 1 && !locked ? (
            ...
            <>
              ...
              <Button disabled={!canCreate} onClick={() => create.mutate()}>
                {create.isPending
                  ? t("tools.connections.add.creatingDraft", { defaultValue: "Creating draft…" })
                  : t("tools.connections.add.createAndProbe", { defaultValue: "Create & probe" })}
              </Button>
            </>
          ) : (
            <>
              <Button variant="outline" disabled={probe.isPending} onClick={() => draft && probe.mutate(draft.id)}>
                ...
                {probe.isPending
                  ? t("tools.connections.add.probing", { defaultValue: "Probing…" })
                  : t("tools.connections.add.reprobe", { defaultValue: "Re-probe" })}
              </Button>
              <Button disabled={activate.isPending || probe.isPending} onClick={() => draft && activate.mutate(draft.id)}>
                {activate.isPending
                  ? t("tools.connections.add.activating", { defaultValue: "Activating…" })
                  : t("tools.connections.add.activate", { defaultValue: "Activate" })}
              </Button>
            </>
          )}
        </DialogFooter>
