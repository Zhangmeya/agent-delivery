...
export function RiskBadge({ risk }: { risk: ToolRiskLevel | null | undefined }) {
  ...
  if (!risk) {
    return <Badge variant="outline">{t("tools.shared.risk.unknown", { defaultValue: "unknown" })}</Badge>;
  }
  const variant =
    risk === "high" || risk === "critical"
      ...
      : risk === "medium"
        ? "secondary"
        : "outline";
  return <Badge variant={variant}>{t(`tools.shared.risk.${risk}`, { defaultValue: risk })}</Badge>;
}
...
export function CapabilityBadges({
  ...
  return (
    <span className="inline-flex flex-wrap gap-1">
      {isReadOnly ? <Badge variant="outline">{t("tools.shared.capability.readOnly", { defaultValue: "read-only" })}</Badge> : null}
      {isWrite ? <Badge variant="secondary">{t("tools.shared.capability.write", { defaultValue: "write" })}</Badge> : null}
      {isDestructive ? <Badge variant="destructive">{t("tools.shared.capability.destructive", { defaultValue: "destructive" })}</Badge> : null}
    </span>
  );
}
...
export function HealthBadge({
  ...
  return <StatusBadge status={healthToStatusKey(raw)} label={t(`tools.shared.health.${raw}`, { defaultValue: label ?? raw })} />;
}
...
export function DecisionBadge({ decision }: { decision: ToolPolicyDecision | string | null | undefined }) {
  ...
  return <StatusBadge status={key} label={t(`tools.shared.decision.${key}`, { defaultValue: label })} />;
}

/** Compact relative time, falling back to absolute. */
export function RelativeTime({ value }: { value: Date | string | null | undefined }) {
  const { t } = useTranslation();
  if (!value) return <span className="text-muted-foreground">{t("tools.shared.time.never", { defaultValue: "never" })}</span>;
  ...
  let text: string;
  if (mins < 1) text = t("tools.shared.time.justNow", { defaultValue: "just now" });
  else {
    const value =
      mins < 60 ? `${mins}m` : mins < 1440 ? `${Math.round(mins / 60)}h` : `${Math.round(mins / 1440)}d`;
    text = isFuture
      ? t("tools.shared.time.in", { defaultValue: "in {{value}}", value })
      : t("tools.shared.time.ago", { defaultValue: "{{value}} ago", value });
  }
  return (
    <span title={date.toLocaleString(getCurrentLocale())} className="text-muted-foreground">
      {text}
    </span>
  );
}
...
export function LoadingState({ label }: { label?: string }) {
  ...
  return (
    <div className="flex items-center gap-2 py-10 text-sm text-muted-foreground">
      ...
      {label ?? t("tools.shared.loading", { defaultValue: "Loading…" })}
    </div>
  );
}
...
export function ErrorState({ error, onRetry }: { error: unknown; onRetry?: () => void }) {
  ...
  if (error instanceof ApiError) {
    if (error.status === 403) {
      message = t("tools.shared.error.forbidden", {
      ...
      message = t("tools.shared.error.unavailable", {
    ...
    message = t("tools.shared.error.generic", { defaultValue: "Something went wrong." });
  }
  return (
    <Card className="border-destructive/40">
      <CardContent className="flex flex-col gap-3 py-6">
        <div className="flex items-start gap-2 text-sm text-destructive">
          ...
          <div>
            <p className="font-medium">{t("tools.shared.error.loadView", { defaultValue: "Could not load this view" })}</p>
        ...
        {onRetry ? (
          ...
          >
            {t("tools.shared.retry", { defaultValue: "Retry" })}
          </button>
        ) : null}
      </CardContent>
    </Card>
      ...
      <CardContent className="flex flex-col gap-2 py-8">
        ...
        {issue ? (
          <a href={issue.href} className="text-sm font-medium text-primary hover:underline">
            {t("tools.shared.trackedIn", { defaultValue: "Tracked in {{identifier}} →", identifier: issue.identifier })}
          </a>
        ) : null}
      </CardContent>
    </Card>
