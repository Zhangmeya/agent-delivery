...
export function RunYourOwnTab({ companyId }: { companyId: string }) {
  ...
  const createMutation = useMutation({
    ...
    onSuccess: () => {
      pushToast({
        title: t("tools.runYourOwn.toolAdded", { defaultValue: "Tool added" }),
        body: t("tools.runYourOwn.toolReady", {
          defaultValue: '"{{name}}" is ready to connect.',
          name: name.trim(),
        }),
        tone: "success",
      });
  ...
  return (
    <div className="space-y-6">
      <p className="max-w-2xl text-sm text-muted-foreground">
        {t("tools.runYourOwn.description", {
          defaultValue:
            "For a tool that runs from a command. Paperclip runs it in your company's own isolated workspace. Administrators only.",
        })}
      </p>

      <div className="space-y-5 rounded-lg border border-border bg-card p-5">
        <div className="space-y-1.5">
          <Label htmlFor="ryo-name">
            {t("tools.runYourOwn.name", { defaultValue: "Name" })}
          </Label>
          <Input
            id="ryo-name"
            value={name}
            onChange={(event) => setName(event.target.value)}
            placeholder={t("tools.runYourOwn.namePlaceholder", { defaultValue: "Acme tools" })}
            maxLength={160}
          />
          <p className="text-xs text-muted-foreground">
            {t("tools.runYourOwn.nameHelp", {
              defaultValue: "What you'll call this tool in your apps list.",
            })}
          </p>
        </div>

        <div className="space-y-1.5">
          <Label htmlFor="ryo-command">
            {t("tools.runYourOwn.command", { defaultValue: "Command" })}
          </Label>
          <Input
            id="ryo-command"
            value={command}
          ...
          />
          <p className="text-xs text-muted-foreground">
            {t("tools.runYourOwn.commandHelp", {
              defaultValue: "The command that runs the tool. From the tool's README.",
            })}
          </p>
        </div>
        ...
        <div className="space-y-2">
          <div className="flex items-baseline gap-2">
            <Label>{t("tools.runYourOwn.requiredKeys", { defaultValue: "Keys it needs" })}</Label>
            <span className="text-xs text-muted-foreground">
              · {t("tools.runYourOwn.keysOptional", { defaultValue: "optional, depends on the tool" })}
            </span>
          </div>
          {keyRows.length > 0 ? (
            <div className="space-y-2">
                  ...
                  <div key={row.id} className="space-y-1">
                    <div className="flex items-center gap-2">
                      ...
                      <Button
                        ...
                        variant="ghost"
                        size="icon"
                        aria-label={t("tools.runYourOwn.removeKey", { defaultValue: "Remove key" })}
          ...
          <Button type="button" variant="outline" size="sm" onClick={addKeyRow} className="gap-1.5">
            ...
            {t("tools.runYourOwn.addKey", { defaultValue: "Add a key" })}
          </Button>
        </div>
        ...
        <div className="flex items-start gap-2.5 rounded-md bg-muted/50 px-3 py-2.5">
          ...
          <div className="text-xs">
            <p className="font-medium text-foreground">
              {t("tools.runYourOwn.isolationNotice", {
                defaultValue: "This runs in your company's own workspace, isolated from everything else.",
              })}
            </p>
            <p className="mt-0.5 flex items-center gap-1 text-muted-foreground">
              <Lock className="h-3 w-3" />
              {t("tools.runYourOwn.adminOnly", {
                defaultValue: "Only administrators see this option.",
              })}
            </p>
          </div>
        </div>
        ...
        <div className="flex flex-wrap items-center gap-3">
          <Button onClick={() => createMutation.mutate()} disabled={!canSubmit || createMutation.isPending}>
            {createMutation.isPending
              ? t("tools.runYourOwn.adding", { defaultValue: "Adding…" })
              : t("tools.runYourOwn.checkAndContinue", { defaultValue: "Check & continue" })}
          </Button>
          <span className="text-xs text-muted-foreground">
            {t("tools.runYourOwn.registrationNotice", {
              defaultValue: "Paperclip will register the command and the keys it needs.",
            })}
          </span>
        </div>
      </div>
      ...
      <div className="space-y-3">
        <h3 className="text-sm font-semibold text-foreground">
          {t("tools.runYourOwn.yourTools", { defaultValue: "Your own tools" })}
        </h3>
        {templates.isLoading ? (
          ...
          <p className="text-sm text-muted-foreground">
            {t("tools.runYourOwn.empty", {
              defaultValue: "You haven't added any of your own tools yet.",
            })}
          </p>
        ) : (
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-border bg-muted/40 text-left text-(length:--text-micro) font-semibold uppercase tracking-wide text-muted-foreground">
                  ...
                  <th className="px-4 py-2.5">
                    {t("tools.runYourOwn.keys", { defaultValue: "Keys" })}
                  </th>
                  <th className="px-4 py-2.5">
                    {t("tools.runYourOwn.added", { defaultValue: "Added" })}
                  </th>
                  <th className="px-4 py-2.5" />
                </tr>
              </thead>
...
function RunYourOwnRow({
  ...
  const disableMutation = useMutation({
    ...
    onSuccess: () => {
      pushToast({
        title: t("tools.runYourOwn.toolTurnedOff", { defaultValue: "Tool turned off" }),
        tone: "success",
      });
      qc.invalidateQueries({ queryKey: queryKeys.tools.stdioTemplates(companyId) });
    },
    onError: (error) => {
      pushToast({
        title: t("tools.runYourOwn.turnOffFailed", { defaultValue: "Couldn't turn it off" }),
  ...
  return (
    <tr className="border-b border-border last:border-0">
      <td className="px-4 py-3">
        ...
        {disabled ? (
          <Badge variant="outline">{t("tools.runYourOwn.off", { defaultValue: "off" })}</Badge>
        ) : null}
      </td>
      <td className="px-4 py-3">
      ...
      <td className="px-4 py-3 text-muted-foreground">
        {template.envKeys.length > 0
          ? template.envKeys.join(", ")
          : t("tools.runYourOwn.none", { defaultValue: "none" })}
      </td>
      <td className="px-4 py-3">
      ...
      <td className="px-4 py-3 text-right">
        {disabled ? null : (
          ...
          >
            {t("tools.runYourOwn.turnOff", { defaultValue: "Turn off" })}
          </Button>
        )}
      </td>
    </tr>
