...
const STATUS_FILTER_OPTIONS: { value: CaseStatus; labelKey: string; defaultLabel: string }[] = [
  { value: "draft", labelKey: "cases.status.draft", defaultLabel: "Draft" },
  { value: "in_progress", labelKey: "cases.status.inProgress", defaultLabel: "In progress" },
  { value: "in_review", labelKey: "cases.status.inReview", defaultLabel: "In review" },
  { value: "approved", labelKey: "cases.status.approved", defaultLabel: "Approved" },
  { value: "done", labelKey: "cases.status.done", defaultLabel: "Done" },
  { value: "cancelled", labelKey: "cases.status.cancelled", defaultLabel: "Cancelled" },
];
...
const CASE_COLUMN_LABELS: Record<CaseColumn, { key: string; defaultValue: string }> = {
  id: { key: "cases.columns.id", defaultValue: "ID" },
  key: { key: "cases.columns.key", defaultValue: "Key" },
  title: { key: "cases.columns.title", defaultValue: "Title" },
  status: { key: "cases.columns.status", defaultValue: "Status" },
  updated: { key: "cases.columns.updated", defaultValue: "Updated" },
  created: { key: "cases.columns.createdAt", defaultValue: "Created at" },
  type: { key: "cases.columns.type", defaultValue: "Type" },
  project: { key: "cases.columns.project", defaultValue: "Project" },
  parent: { key: "cases.columns.parent", defaultValue: "Parent case" },
};
const CASE_SORT_LABELS: Record<CaseSortField, { key: string; defaultValue: string }> = {
  updated: { key: "cases.sort.lastUpdated", defaultValue: "Last updated" },
  created: { key: "cases.sort.createdAt", defaultValue: "Created at" },
  title: { key: "cases.sort.title", defaultValue: "Title" },
  status: { key: "cases.sort.status", defaultValue: "Status" },
  id: { key: "cases.sort.id", defaultValue: "ID" },
  type: { key: "cases.sort.type", defaultValue: "Type" },
  project: { key: "cases.sort.project", defaultValue: "Project" },
};
    ...
    <Popover open={open} onOpenChange={setOpen}>
      <PopoverTrigger asChild>
        <button
          ...
          aria-label={t("cases.actions.changeStatus", { defaultValue: "Change case status" })}
          onClick={(event) => {
    ...
    <span className="grid min-w-0 flex-1 items-center gap-2" style={{ gridTemplateColumns: caseTrailingGridTemplate(columns) }}>
      {columns.map((column) => {
        if (column === "id") {
          return (
            <CaseCopyableToken
              ...
              label={t("cases.labels.caseId", { defaultValue: "case ID" })}
        ...
        if (column === "key") {
          return row.key ? (
            <CaseCopyableToken
              key={column}
              value={row.key}
              label={t("cases.labels.caseKey", { defaultValue: "case key" })}
              className="font-mono text-xs text-muted-foreground"
              stopPropagation
            />
          ) : (
            <span key={column} className="min-w-0 truncate text-xs text-muted-foreground">
              {t("cases.common.none", { defaultValue: "None" })}
            </span>
          );
        }
        if (column === "title") {
                  ...
                  {childCount > 0 ? (
                    <button
                      ...
                      aria-label={treeCollapsed
                        ? t("cases.tree.expand", { defaultValue: "Expand {{title}}", title: row.title })
                        : t("cases.tree.collapse", { defaultValue: "Collapse {{title}}", title: row.title })}
        ...
        if (column === "project") {
          return (
            <span key={column} className="min-w-0 truncate text-xs text-muted-foreground">
              {projectName ?? t("cases.common.noProject", { defaultValue: "No project" })}
            </span>
          );
        }
        if (column === "parent") {
          return (
            <span key={column} className="min-w-0 truncate font-mono text-xs text-muted-foreground">
              {row.parentCaseId
                ? t("cases.tree.parent", { defaultValue: "Parent" })
                : t("cases.common.none", { defaultValue: "None" })}
            </span>
          );
        }
        ...
        <span className="grid min-w-0 flex-1 items-center gap-2" style={{ gridTemplateColumns: caseTrailingGridTemplate(trailingColumns) }}>
          {trailingColumns.map((column) => (
            <span key={column} className={cn("truncate", (column === "updated" || column === "created") && "text-right")}>
              {t(CASE_COLUMN_LABELS[column].key, { defaultValue: CASE_COLUMN_LABELS[column].defaultValue })}
            </span>
          ))}
        </span>
      ) : null}
        ...
        <IssueGroupHeader
          ...
          trailing={(
            <span className="text-xs text-muted-foreground tabular-nums">
              {count === 1
                ? t("cases.groupCount.one", { defaultValue: "{{count}} case", count })
                : t("cases.groupCount.other", { defaultValue: "{{count}} cases", count })}
            </span>
          )}
        />
      </div>
...
function CaseColumnPicker({
  ...
  return (
    <CaseToolbarButton
      icon={Columns3}
      title={t("cases.toolbar.columns", { defaultValue: "Columns" })}
      active={!sameStringSet([...visibleColumns], DEFAULT_CASE_COLUMNS)}
    >
      <PopoverContent align="end" className="w-(--sz-300px) p-1.5">
        <div className="px-2 pb-1 pt-1.5">
          <div className="text-(length:--text-nano) font-semibold uppercase tracking-(--tracking-caps) text-muted-foreground">
            {t("cases.columns.desktopRows", { defaultValue: "Desktop case rows" })}
          </div>
          <div className="text-sm font-medium text-foreground">
            {t("cases.columns.chooseVisible", { defaultValue: "Choose visible columns" })}
          </div>
        </div>
        <div className="space-y-0.5">
          {CASE_COLUMN_ORDER.map((column) => (
            ...
            >
              <span>{t(CASE_COLUMN_LABELS[column].key, { defaultValue: CASE_COLUMN_LABELS[column].defaultValue })}</span>
        ...
        <div className="mt-1 border-t border-border pt-1">
          <button
            ...
            onClick={onReset}
          >
            {t("cases.columns.resetDefaults", { defaultValue: "Reset defaults" })}
          </button>
        </div>
      </PopoverContent>
    </CaseToolbarButton>
...
function CaseSortPicker({
  ...
  return (
    <CaseToolbarButton
      icon={ArrowUpDown}
      title={t("cases.toolbar.sort", { defaultValue: "Sort" })}
      active={sortField !== "updated" || sortDir !== "desc"}
    >
      <PopoverContent align="end" className="w-48 p-2">
        {(Object.keys(CASE_SORT_LABELS) as CaseSortField[]).map((field) => (
          ...
          >
            <span>{t(CASE_SORT_LABELS[field].key, { defaultValue: CASE_SORT_LABELS[field].defaultValue })}</span>
            {sortField === field ? (
...
function CasesEmptyHero() {
  ...
  return (
    <div className="mx-auto flex max-w-xl flex-col items-center gap-4 py-16 text-center">
      ...
      <h2 className="text-lg font-semibold">{t("cases.empty.title", { defaultValue: "No cases yet" })}</h2>
      <p className="text-sm text-muted-foreground">
        {t("cases.empty.description", {
          defaultValue: "Cases are durable work products — blog posts, tweet storms, docs pages — that tasks create and iterate on. In v1 they're created by agents, not from the UI.",
        })}
      </p>
      <div className="w-full space-y-2 rounded-lg border border-border bg-muted/50 p-4 text-left">
        <p className="text-sm font-medium">
          {t("cases.empty.skillPromptIntro", { defaultValue: "To start creating cases, add this to a skill:" })}
        </p>
...
{slug, target_audience, publish_url} and key <release>/<slug>."`}
        ...
        <p className="text-xs text-muted-foreground">
          {t("cases.empty.skillReferencePrefix", { defaultValue: "See the Paperclip CN skill →" })}{" "}
          <code className="font-mono">references/cases.md</code>{" "}
          {t("cases.empty.skillReferenceSuffix", { defaultValue: "for the API." })}
        </p>
      </div>
      <p className="text-xs text-muted-foreground">
        {t("cases.empty.featureFlagPrefix", { defaultValue: "Feature is gated by the" })}{" "}
        <code className="font-mono">enableCases</code>{" "}
        {t("cases.empty.featureFlagSuffix", { defaultValue: "experimental flag (Settings → Experimental)." })}
      </p>
    </div>
  );
}
...
export function Cases() {
  ...
  useEffect(() => {
    setBreadcrumbs([{ label: t("cases.title", { defaultValue: "Cases" }) }]);
  }, [setBreadcrumbs, t]);
  ...
  const groupedRows = useMemo((): CaseGroupedRows[] => {
    ...
    for (const c of sorted) {
      ...
      if (viewState.groupBy === "type") key = c.caseType;
        ...
        const option = STATUS_FILTER_OPTIONS.find((entry) => entry.value === c.status);
        key = option ? t(option.labelKey, { defaultValue: option.defaultLabel }) : c.status;
      } else {
        key = c.projectId
          ? projectName.get(c.projectId) ?? t("cases.common.unknownProject", { defaultValue: "Unknown project" })
          : t("cases.common.noProject", { defaultValue: "No project" });
      }
  ...
  }, [projectName, sorted, t, viewState.groupBy]);
  ...
  if (viewState.search.trim()) {
    activeFilters.push({
      key: "search",
      label: t("cases.filters.search", { defaultValue: "Search" }),
      value: viewState.search.trim(),
    });
  }
  if (viewState.typeFilters.length > 0) {
    activeFilters.push({
      key: "type",
      label: t("cases.filters.type", { defaultValue: "Type" }),
      value: viewState.typeFilters.join(", "),
    });
  }
  if (!usesDefaultStatusFilter) {
    activeFilters.push({
      key: "status",
      label: t("cases.filters.status", { defaultValue: "Status" }),
      value: viewState.statusFilters.length === CASE_STATUSES.length
        ? t("cases.filters.all", { defaultValue: "All" })
        : viewState.statusFilters
          .map((status) => {
            const option = STATUS_FILTER_OPTIONS.find((entry) => entry.value === status);
            return option ? t(option.labelKey, { defaultValue: option.defaultLabel }) : status;
          })
          .join(", "),
    });
  }
  if (viewState.projectFilters.length > 0) {
    activeFilters.push({
      key: "project",
      label: t("cases.filters.project", { defaultValue: "Project" }),
      value: viewState.projectFilters
        .map((projectId) => projectId === ALL
          ? t("cases.common.noProject", { defaultValue: "No project" })
          : projectName.get(projectId) ?? t("cases.filters.project", { defaultValue: "Project" }))
        .join(", "),
    });
  }
  if (viewState.labelFilter !== ALL) {
    const name = (labelsQuery.data ?? []).find((l) => l.id === viewState.labelFilter)?.name
      ?? t("cases.filters.label", { defaultValue: "Label" });
    activeFilters.push({
      key: "label",
      label: t("cases.filters.label", { defaultValue: "Label" }),
      value: name,
    });
  }
    ...
    <div className="space-y-4">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div className="flex items-center gap-2">
          <h1 className="text-xl font-bold">{t("cases.title", { defaultValue: "Cases" })}</h1>
          <Badge variant="secondary">{t("cases.experimental", { defaultValue: "Experimental" })}</Badge>
        </div>
      </div>
      ...
      {noCasesAtAll ? (
        ...
        <>
          <div className="flex items-center justify-between gap-2 sm:gap-3">
            <div className="relative w-48 sm:w-64 md:w-80">
              ...
              <Input
                ...
                placeholder={t("cases.search.placeholder", { defaultValue: "Search cases..." })}
                className="pl-7 text-xs sm:text-sm"
                aria-label={t("cases.search.ariaLabel", { defaultValue: "Search cases" })}
            ...
            <div className="flex shrink-0 items-center gap-0.5 sm:gap-1">
              <Button
                ...
                title={viewState.treeView
                  ? t("cases.tree.showFlat", { defaultValue: "Show flat case list" })
                  : t("cases.tree.showHierarchy", { defaultValue: "Show parent/children tree" })}
              ...
              <CaseToolbarButton
                icon={Filter}
                title={t("cases.toolbar.filters", { defaultValue: "Filters" })}
                active={hasActiveFilters}
              >
                <PopoverContent align="end" className="w-72 p-3">
                  <div className="grid gap-3">
                    <FilterField label={t("cases.filters.type", { defaultValue: "Type" })}>
                      <div className="max-h-40 overflow-y-auto">
                        {distinctTypes.length === 0 ? (
                          <p className="px-1 py-1 text-xs text-muted-foreground">
                            {t("cases.filters.noTypes", { defaultValue: "No types yet" })}
                          </p>
                    ...
                    <FilterField label={t("cases.filters.status", { defaultValue: "Status" })}>
                      <div>
                        {STATUS_FILTER_OPTIONS.map((option) => (
                          <FilterCheckboxRow
                            key={option.value}
                    ...
                    <FilterField label={t("cases.filters.project", { defaultValue: "Project" })}>
                      <div className="max-h-40 overflow-y-auto">
                        <FilterCheckboxRow
                          label={t("cases.common.noProject", { defaultValue: "No project" })}
                    ...
                    <FilterField label={t("cases.filters.label", { defaultValue: "Label" })}>
                      <div className="max-h-40 overflow-y-auto">
                        <FilterCheckboxRow
                          label={t("cases.filters.allLabels", { defaultValue: "All labels" })}
                    ...
                    <Button type="button" variant="ghost" size="sm" onClick={clearFilters} disabled={!hasActiveFilters}>
                      {t("cases.filters.clear", { defaultValue: "Clear filters" })}
                    </Button>
                  </div>
                </PopoverContent>
              </CaseToolbarButton>
              ...
              <CaseToolbarButton
                icon={Layers}
                title={t("cases.toolbar.group", { defaultValue: "Group" })}
                active={viewState.groupBy !== "type"}
              >
                <PopoverContent align="end" className="w-44 p-2">
                  {([
                    ["type", t("cases.group.type", { defaultValue: "Type" })],
                    ["project", t("cases.group.project", { defaultValue: "Project" })],
                    ["status", t("cases.group.status", { defaultValue: "Status" })],
                    ["none", t("cases.group.none", { defaultValue: "None" })],
          ...
          {filtered.length === 0 ? (
            <EmptyState
              icon={SearchX}
              message={t("cases.filteredEmpty.message", { defaultValue: "No cases match these filters." })}
              action={t("cases.filters.clear", { defaultValue: "Clear filters" })}
