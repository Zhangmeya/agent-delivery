...
export interface SkillDenial {
  ...
  title: string;
  /** Locale key for application-owned title copy. */
  titleKey: string;
  /** Interpolation values for the localized title. */
  titleValues?: Record<string, string>;
  /** Human remediation — never a curl/API-key snippet. */
  remediation: string;
  /** Locale key when remediation came from an application-owned default. */
...
const PLATFORM_COPY_KEYS: Record<string, { title: string; remediation: string }> = {
  skill_authentication_required: {
    title: "skillPolicyDenial.authentication.title",
    remediation: "skillPolicyDenial.authentication.remediation",
  },
  skill_company_boundary_denied: {
    title: "skillPolicyDenial.companyBoundary.title",
    remediation: "skillPolicyDenial.companyBoundary.remediation",
  },
  skill_workspace_boundary_denied: {
    title: "skillPolicyDenial.workspaceBoundary.title",
    remediation: "skillPolicyDenial.workspaceBoundary.remediation",
  },
  skill_source_validation_failed: {
    title: "skillPolicyDenial.sourceValidation.title",
    remediation: "skillPolicyDenial.sourceValidation.remediation",
  },
  skill_unsafe_content_blocked: {
    title: "skillPolicyDenial.unsafeContent.title",
    remediation: "skillPolicyDenial.unsafeContent.remediation",
  },
  skill_secret_handling_blocked: {
    title: "skillPolicyDenial.secretHandling.title",
    remediation: "skillPolicyDenial.secretHandling.remediation",
  },
  skill_actor_restricted: {
    title: "skillPolicyDenial.actorRestricted.title",
    remediation: "skillPolicyDenial.actorRestricted.remediation",
  },
};
...
export function classifySkillDenial(
  ...
  if (isPolicyDenial) {
    ...
    return {
      ...
      reason,
      title,
      titleKey: actionLabel
        ? "skillPolicyDenial.policy.actionTitle"
        : "skillPolicyDenial.policy.title",
      ...(actionLabel ? { titleValues: { action: actionLabel } } : {}),
      remediation: remediation ?? DEFAULT_POLICY_REMEDIATION,
      ...(remediation ? {} : { remediationKey: "skillPolicyDenial.policy.remediation" }),
    };
  }
  ...
  if (code === SKILL_POLICY_ADMIN_CODE) {
    return {
      ...
      titleKey: "skillPolicyDenial.admin.title",
      remediation: remediation ?? DEFAULT_ADMIN_REMEDIATION,
      ...(remediation ? {} : { remediationKey: "skillPolicyDenial.admin.remediation" }),
    };
  }
  ...
  if (isPlatformInvariant) {
    const copyKeys = (code ? PLATFORM_COPY_KEYS[code] : undefined) ?? {
      title: "skillPolicyDenial.platform.title",
      remediation: "skillPolicyDenial.platform.remediation",
    };
    return {
      state: "platform",
      code,
      reason,
      title: (code ? PLATFORM_TITLES[code] : undefined) ?? "This action is blocked by a platform safety rule.",
      titleKey: copyKeys.title,
      remediation:
        remediation
        ?? (code ? PLATFORM_REMEDIATIONS[code] : undefined)
        ?? "Fix the flagged issue and try again.",
      ...(remediation ? {} : { remediationKey: copyKeys.remediation }),
    };
  }
