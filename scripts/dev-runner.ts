...
async function getMigrationStatusPayload() {
  ...
  if (status.code !== 0) {
    process.stderr.write(
      status.stderr ||
        status.stdout ||
        `[paperclip] Command failed with code ${status.code}: pnpm --filter @penclipai/db exec tsx src/migration-status.ts --json\n`,
    );
...
async function buildPluginSdk() {
  ...
  const result = await runPnpm(
    ["--filter", "@penclipai/plugin-sdk", "build"],
    { stdio: "inherit" },
  );
  if (result.signal) {
...
async function startServerChild() {
  ...
  child = spawn(
    pnpmBin,
    ["--filter", "@penclipai/server", serverScript, ...forwardedArgs],
