...
Font assets live in Vite's public directory so `pnpm --filter @penclipai/ui build`
...
```sh
pnpm --filter @penclipai/skills-catalog validate
```
...
```sh
pnpm --filter @penclipai/skills-catalog build:manifest
```

The package's `build` script runs `build:manifest` and then `tsc`; tests live
under `pnpm --filter @penclipai/skills-catalog test`. Validation fails when:
