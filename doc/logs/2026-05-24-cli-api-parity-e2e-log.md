...
- Actual result: Runbook requires explicit scratch paths, port `3197`, unset database env vars, `pnpm penclip onboard --yes --run --bind loopback`, and pre-test isolation checks.
- Status: PASS.
...
- Command: `env -u DATABASE_URL -u DATABASE_MIGRATION_URL ... pnpm penclip onboard --yes --run --bind loopback`
...
- Command: `pnpm penclip env`; `pnpm penclip context show --json`; `curl -sS http://127.0.0.1:3197/api/health`; `find tmp/cli-api-parity/home -maxdepth 4 -type d`
...
- Status: PASS.
- Output summary: `penclip env` redacted here because it prints the generated `PAPERCLIP_AGENT_JWT_SECRET`. Context existed at the scratch path with an empty `default` profile.
- Follow-up: Set API context and begin CLI parity checks.

### 2026-05-24T11:08:20+02:00 - Basic context/auth/connectivity

- Command: `pnpm penclip context set --api-base http://127.0.0.1:3197 --use --json`; `pnpm penclip whoami --json`; `pnpm penclip company list --json`; `pnpm penclip access whoami --json`
...
- Command: `pnpm penclip company create --payload-json '{"name":"CLI API Parity Test","description":"Disposable company for CLI API parity testing","goal":"Exercise the CLI API surface end to end"}' --json`; `pnpm penclip context set --company-id 12e9db4b-f66c-459b-959e-d645002240fb --use --json`; `pnpm penclip company get 12e9db4b-f66c-459b-959e-d645002240fb --json`; `pnpm penclip company update 12e9db4b-f66c-459b-959e-d645002240fb --payload-json '{"description":"Updated by CLI API parity test","budgetMonthlyCents":12345}' --json`
...
- Command: edited `cli/src/commands/client/context.ts`, `cli/src/client/context.ts`, and `cli/src/__tests__/context.test.ts`; `pnpm exec vitest run cli/src/__tests__/context.test.ts`; `pnpm --dir cli typecheck`; `pnpm penclip context set --api-base http://127.0.0.1:3197 --company-id 12e9db4b-f66c-459b-959e-d645002240fb --use --json`; `pnpm penclip context show --json`
...
- Command: `kill <paperclip pid on 3197>`; `pnpm penclip onboard --yes --run --bind loopback`; `curl http://127.0.0.1:3197/api/health`; `issue tree-hold:get <issue-id> null --json`
...
- Command: `curl -sf http://127.0.0.1:3197/api/health`; `pnpm penclip health --json`; `pnpm penclip adapter list/get/config-schema/ui-parser/models/model-profiles/detect-model/test-environment ... --json`; `pnpm penclip environment list/capabilities/probe-config/create ... --json`; `pnpm penclip project-workspace create/list/update/delete ... --json`; `pnpm penclip workspace list --company-id ... --json`; `pnpm penclip plugin list/examples/ui-contributions/tools/init ... --json`
...
- Command: `pnpm penclip asset image:upload --company-id <company-id> --file doc/assets/avatars/zinc.png --namespace cli-parity --alt ... --title ... --json`; `pnpm penclip asset content <asset-id> --out tmp/cli-api-parity/artifacts/asset-download.png --json`; `pnpm penclip asset logo:upload --company-id <company-id> --file ui/public/favicon-32x32.png --json`; `pnpm penclip skill list/create/get/file/file:update/update-status/install-update/delete/scan-projects ... --json`
...
- Command: `env -u DATABASE_URL -u DATABASE_MIGRATION_URL ... pnpm penclip environment create --company-id 12e9db4b-f66c-459b-959e-d645002240fb --payload-json '{"name":"CLI parity local env","description":"Disposable CLI parity environment","driver":"local","config":{"cwd":"/Users/aronprins/Documents/PaperclipAI/paperclip"}}' --json`
- Purpose: Verify `BUG-004` against the restarted isolated source-tree server.
- Prerequisites/IDs used: Same scratch env; server restarted with `pnpm penclip onboard --yes --run --bind loopback`; company `12e9db4b-f66c-459b-959e-d645002240fb`.
...
- Command: `pnpm penclip environment create/get/leases/probe/update/delete ... --json`; `pnpm penclip plugin install/list/inspect/health/config/jobs/local-folders/ui-contributions/disable/enable/uninstall ... --json`; `pnpm penclip secrets list/create/link/declarations/migrate-inline-env ... --json`
...
- Command: `pnpm penclip secrets update 20c74546-7bec-4766-80cd-0b6c57545f7d --payload-json ... --json`; `pnpm penclip secrets rotate 20c74546-7bec-4766-80cd-0b6c57545f7d --value ... --json`; `pnpm penclip secrets usage 20c74546-7bec-4766-80cd-0b6c57545f7d --json`; `pnpm penclip secrets access-events 20c74546-7bec-4766-80cd-0b6c57545f7d --json`; `pnpm penclip secrets delete 20c74546-7bec-4766-80cd-0b6c57545f7d --yes --confirm 20c74546-7bec-4766-80cd-0b6c57545f7d --json`; `pnpm penclip secrets list --company-id 12e9db4b-f66c-459b-959e-d645002240fb --json`
...
- Expected result: `penclip health` exists; `penclip access whoami` works; `invite test-resolution` has a URL option; `join list --status pending` maps to `pending_approval`; issue help text no longer overstates valid cancel/recovery inputs.
...
- Command: `pnpm penclip health --json`; `pnpm penclip access whoami --json`; `pnpm penclip join list --company-id 12e9db4b-f66c-459b-959e-d645002240fb --status pending --request-type agent --json`; `pnpm penclip invite create --company-id 12e9db4b-f66c-459b-959e-d645002240fb --payload-json '{}' --json`; `pnpm penclip invite test-resolution <token> --url https://example.com/invite/<token> --json`; `pnpm penclip invite revoke <invite-id> --json`; `pnpm penclip issue recovery:resolve --help`; `pnpm penclip issue interaction:cancel --help`
...
- Command: `pnpm penclip llm agent-configuration --json`; `pnpm penclip llm agent-icons --json`; `pnpm penclip llm agent-configuration:adapter process --json`; `pnpm penclip available-skill list --json`; `pnpm penclip available-skill get paperclip --json`; `pnpm penclip available-skill get cmux --json`; `pnpm penclip openapi --json`
...
- Command: environment echo; `pnpm penclip health --json`; `pnpm penclip token board list --json`; `pnpm penclip token board revoke <redacted-board-token-id> --json`; `pnpm penclip token agent list --company-id 12e9db4b-f66c-459b-959e-d645002240fb --agent 1dd601a1-031a-4225-b005-419427fd059f --json`; `pnpm penclip plugin list --json`; `pnpm penclip secrets list --company-id 12e9db4b-f66c-459b-959e-d645002240fb --json`; `pnpm penclip environment list --company-id 12e9db4b-f66c-459b-959e-d645002240fb --json`; `pnpm penclip project-workspace list d32032ce-d95e-4c4e-a942-dd98498025fb --json`; `pnpm penclip openapi --json`
...
- Command: `pnpm penclip doctor --config <scratch-config>`; `pnpm penclip doctor --config <scratch-config> --repair --yes`; `pnpm penclip env --config <scratch-config>`; `pnpm penclip db:backup --config <scratch-config> --dir tmp/cli-api-parity/artifacts/root-setup/backups --retention-days 1 --filename-prefix cli-parity --json`; `pnpm penclip allowed-hostname cli-parity.test --config <scratch-config>`; `pnpm penclip auth bootstrap-ceo --config <scratch-config> --force --base-url http://127.0.0.1:3197`; `pnpm penclip auth whoami --json`; `pnpm penclip routines disable-all --config <scratch-config> --company-id 12e9db4b-f66c-459b-959e-d645002240fb --json`; `pnpm penclip env-lab doctor --instance cli-api-parity --json`; `pnpm penclip env-lab status --instance cli-api-parity --json`
...
- Command: `PAPERCLIP_WORKTREES_DIR=tmp/cli-api-parity/worktrees-home pnpm penclip worktree:list --json`; `pnpm penclip worktree env --config <scratch-config> --json`; `pnpm penclip worktree:merge-history --from current --to current --company CLI --dry`; `pnpm penclip cloud push --company 12e9db4b-f66c-459b-959e-d645002240fb --dry-run --json`
...
- Actual result: `worktree:list` passed and showed the current repo branch `improvement/cli-api-parity` with no Paperclip worktree config. `worktree env --config <scratch-config> --json` passed and printed the scratch `PAPERCLIP_CONFIG` plus generated env values; the generated JWT secret is intentionally not copied into this log. `worktree:merge-history --from current --to current --company CLI --dry` failed as expected with `Source and target Paperclip configs are the same. Choose different --from/--to worktrees.` `cloud push --dry-run` failed as expected with `Cloud sync is disabled. Enable the cloud sync experimental setting before running penclip cloud push.`
...
- Command: `HOME=tmp/cli-api-parity/shell-home PAPERCLIP_WORKTREES_DIR=tmp/cli-api-parity/worktree-instances pnpm penclip worktree:make cli-parity-wt --home <scratch-worktree-home> --from-config <scratch-config> --server-port 3198 --db-port 54331 --seed-mode minimal`; `pkill` only for the runaway scratch install attempt; edited `cli/src/commands/worktree.ts` and `cli/src/__tests__/worktree.test.ts`; `pnpm exec vitest run cli/src/__tests__/worktree.test.ts`; `pnpm --dir cli typecheck`; `penclip worktree:cleanup cli-parity-wt --home <scratch-worktree-home> --force`; reran `penclip worktree:make ...`; `penclip worktree:list --json`; `penclip worktree env --config <scratch-worktree-config> --json`; `penclip worktree:merge-history --from paperclip-cli-parity-wt --to current --company CLI --dry`
...
- Expected result: `/api/openapi.json` and `penclip openapi --json` return a valid OpenAPI 3.0 document with the reference route inventory, including representative CLI/API parity paths such as `/api/companies/{companyId}/agents` and `/api/agents/{id}/keys`.
...
- Command: Edited `cli/src/commands/client/agent.ts`; ran `pnpm exec vitest run cli/src/__tests__/agent-lifecycle.test.ts`; `pnpm --dir cli typecheck`; `pnpm --silent penclip agent instructions-path:update --help`.
...
- Command: `pnpm add @asteasolutions/zod-to-openapi@7.3.4 --filter @penclipai/server`; replaced `server/src/routes/openapi.ts` inventory stub with the schema-backed generator from `doc/plans/2026-05-23-cli-api-parity-openapi-reference.ts`; added route wrapper exports; tightened `server/src/__tests__/openapi-routes.test.ts`; `pnpm exec vitest run server/src/__tests__/openapi-routes.test.ts`; `pnpm --dir server typecheck`; restarted isolated server with the scratch env; `pnpm penclip openapi --json`.
...
- Expected result: `/api/openapi.json` and `penclip openapi --json` return OpenAPI 3.0 with schema-backed request bodies, security schemes, public-operation security overrides, and create-operation `201` responses.
...
- Command: `pnpm penclip health --json`; `pnpm penclip openapi --json`; `lsof -nP -iTCP:3197 -sTCP:LISTEN`; `git status --short --branch`, all with the scratch `PAPERCLIP_*`, `CODEX_HOME`, and `CLAUDE_HOME` environment and with `DATABASE_URL`/`DATABASE_MIGRATION_URL` unset.
...
- Command: Stopped the foreground scratch server process; started the same runbook command in detached screen session `paperclip-cli-parity`; verified `pnpm penclip health --json`; checked `lsof -nP -iTCP:3197 -sTCP:LISTEN`; checked `screen -ls`.
...
- Reproduction command: Install `packages/plugins/examples/plugin-kitchen-sink-example`, then run `pnpm penclip plugin tool:execute --payload-json '{"tool":"paperclip-kitchen-sink-example:echo","parameters":{"message":"CLI parity tool"},"runContext":{"companyId":"<company-id>","projectId":"<project-id>","agentId":"<agent-id>","runId":"<run-id>"}}' --json`.
...
- Reproduction command: `pnpm penclip token agent list --company-id 12e9db4b-f66c-459b-959e-d645002240fb --agent 1dd601a1-031a-4225-b005-419427fd059f --json`.
...
- Reproduction command: `pnpm penclip configure --config /Users/aronprins/Documents/PaperclipAI/paperclip/tmp/cli-api-parity/home/instances/cli-api-parity/config.json --section invalid-section`.
...
- Reproduction command: `pnpm penclip context set --api-base http://127.0.0.1:3197 --use --json`; then `pnpm penclip context set --company-id <company-id> --use --json`; then `pnpm penclip context show --json`.
...
- Verification command: `pnpm exec vitest run cli/src/__tests__/context.test.ts`; `pnpm --dir cli typecheck`; `pnpm penclip context show --json`.
...
- Reproduction command: `pnpm penclip access whoami --json`.
...
- Fix summary: Added `penclip access whoami` as an alias for the existing top-level `whoami` command.
- Verification command: `pnpm exec vitest run cli/src/__tests__/access-parity.test.ts`; `pnpm --dir cli typecheck`; `pnpm penclip access whoami --json`.
- Remaining risk: Low.
...
- Reproduction command: `pnpm penclip issue interaction:accept <issue-id> <request-confirmation-interaction-id> --json`.
...
- Reproduction command: `pnpm penclip issue tree-hold:get <issue-id> null --json` or `pnpm penclip issue tree-hold:release <issue-id> null --json`.
...
- Reproduction command: `pnpm penclip issue interaction:cancel <issue-id> <request-confirmation-interaction-id> --reason "..." --json`.
...
- Verification command: `pnpm exec vitest run cli/src/__tests__/issue-subresources.test.ts`; `pnpm --dir cli typecheck`; `pnpm penclip issue interaction:cancel --help`.
...
- Status: Fixed help text.
- Severity: Low command UX drift.
- Reproduction command: `pnpm penclip issue recovery:resolve <issue-id> --action-id <action-id> --outcome restored --source-issue-status blocked --json`.
...
- Verification command: `pnpm exec vitest run cli/src/__tests__/issue-subresources.test.ts`; `pnpm --dir cli typecheck`; `pnpm penclip issue recovery:resolve --help`.
...
- Severity: Low command UX drift.
- Reproduction command: `pnpm penclip agent instructions-path:update <process-agent-id> --payload-json '{"path":"docs/cli-parity.md"}' --json`.
...
- Verification command: `pnpm exec vitest run cli/src/__tests__/agent-lifecycle.test.ts`; `pnpm --dir cli typecheck`; `pnpm penclip agent instructions-path:update --help`.
...
- Reproduction command: `pnpm penclip invite test-resolution <invite-token> --json`.
...
- Verification command: `pnpm exec vitest run cli/src/__tests__/access-parity.test.ts`; `pnpm --dir cli typecheck`; `pnpm penclip invite test-resolution <token> --url https://example.com/invite/<token> --json`.
...
- Status: Fixed and live-verified.
- Severity: Low command UX drift.
- Reproduction command: `pnpm penclip join list --company-id <company-id> --status pending --request-type agent --json`.
...
- Verification command: `pnpm exec vitest run cli/src/__tests__/access-parity.test.ts`; `pnpm --dir cli typecheck`; `pnpm penclip join list --company-id <company-id> --status pending --request-type agent --json`.
- Remaining risk: Low.
...
- Reproduction command: `pnpm penclip openapi --json`; `pnpm penclip available-skill get cmux --json`; `pnpm penclip llm agent-configuration --json`; `pnpm penclip llm agent-icons --json`; `pnpm penclip llm agent-configuration:adapter process --json`.
...
- Reproduction command: `CLAUDE_HOME=/Users/aronprins/Documents/PaperclipAI/paperclip/tmp/cli-api-parity/claude-home pnpm penclip available-skill list --json`.
...
### MISMATCH-008 - `penclip health` is not registered

- Status: Fixed and live-verified.
- Severity: Low command/API parity gap.
- Reproduction command: `pnpm penclip health --json`.
...
- Verification command: `pnpm exec vitest run cli/src/__tests__/access-parity.test.ts`; `pnpm --dir cli typecheck`; `pnpm penclip health --json`.
- Remaining risk: Low.
...
- Reproduction command: `pnpm penclip environment create --company-id 12e9db4b-f66c-459b-959e-d645002240fb --payload-json '{"name":"CLI parity local env","description":"Disposable CLI parity environment","driver":"local","config":{"cwd":"/Users/aronprins/Documents/PaperclipAI/paperclip"}}' --json`.
...
- Status: Fixed and live-verified.
- Severity: Medium CLI/API parity gap.
- Reproduction command: `pnpm penclip secrets --help` did not expose commands for `PATCH /api/secrets/:id`, `POST /api/secrets/:id/rotate`, `GET /api/secrets/:id/usage`, `GET /api/secrets/:id/access-events`, or `DELETE /api/secrets/:id`.
