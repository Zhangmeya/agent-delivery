...
- `penclip connect` must show the resolved API base before any auth or mutation and allow the user to override it.
...
```sh
penclip token board create --company-id <company-id> --name "external-admin"
penclip token board list
penclip token board revoke <key-id>
```
...
```sh
penclip token agent create --company-id <company-id> --agent <agent-id-or-name> --name "external-worker"
penclip token agent list --company-id <company-id> --agent <agent-id-or-name>
penclip token agent revoke --agent <agent-id-or-name> <key-id>
```
...
```sh
penclip agent-prompt <agent-name-or-id> <agent-api-key> "Prompt here"
```

Recommended safer variants:

```sh
penclip agent prompt --agent <agent-name-or-id> --api-key-env PAPERCLIP_API_KEY "Prompt here"
penclip agent prompt --profile my-agent "Prompt here"
penclip board prompt --agent <agent-name-or-id> "Prompt here"
```
...
```sh
penclip agent wake <agent>
penclip run list --company-id <company-id>
penclip run get <run-id>
penclip run log <run-id>
penclip run cancel <run-id>
penclip issue runs <issue-id>
```
...
```sh
penclip project list|get|create|update|delete
penclip goal list|get|create|update|delete
```
...
```sh
penclip issue child create <issue-id>
penclip issue document list|get|put|delete|lock|unlock|revisions|restore
penclip issue work-product list|create|update|delete
penclip issue interaction list|create|accept|reject|respond|cancel
penclip issue attachment list|upload|download|delete
penclip issue force-release <issue-id>
penclip issue label list|create|delete
penclip issue read|unread|archive|unarchive
```
...
```sh
penclip agent create|update|pause|resume|approve|terminate|delete
penclip agent org
penclip agent config get|revisions|rollback
penclip agent instructions get|set|file
penclip adapter list|models|profiles|detect|test|install|enable|disable|reload
```
...
```sh
penclip cost summary|by-agent|by-project|by-provider|issue
penclip cost event create
penclip finance event create|list|summary
penclip budget overview|set-company|set-agent|policy-create|incident-resolve
```
...
```sh
penclip invite create|list|revoke|show|onboarding
penclip join list|approve|reject|claim-key
penclip member list|update|archive|permissions
penclip admin user list|promote|demote|company-access
```
...
```sh
penclip routine list|create|get|update|run|runs|trigger|revision
penclip environment list|create|get|update|delete|probe|leases
penclip workspace list|get|update|operations|runtime
penclip project workspace list|create|update|delete|runtime
```
...
```text
penclip connect
penclip token board|agent create|list|revoke
penclip whoami
penclip prompt ...
penclip board ...
penclip agent ...
...
- Add aliases only for high-frequency flows, for example `penclip ask` as an alias for `penclip prompt`.
...
- A new user can run `penclip connect`, confirm or override the API base, select board or agent, and get a saved working profile tied to that API base.
...
```sh
penclip agent-prompt AgentName "$AGENT_API_KEY" "Prompt here"
```
