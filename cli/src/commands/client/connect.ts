...
import type { Agent, Company } from "@penclipai/shared";
import { createAgentKeySchema, createBoardApiKeySchema } from "@penclipai/shared";
...
async function connectWizard(opts: ConnectOptions) {
  if (!process.stdin.isTTY || !process.stdout.isTTY) {
    throw new Error("`penclip connect` is interactive. For scripts, pass --api-base/--api-key or use context set/token commands.");
  }

  p.intro(pc.bgCyan(pc.black(" penclip connect ")));
