...
async function bestEffortCleanup(cleanup: Promise<unknown> | undefined, timeoutMs: number) {
  if (!cleanup) return;
  await Promise.race([
...
async function runCliJson<T>(
  ...
  const cliArgs = ["--silent", "penclip", ...args];
  if (opts.apiBase) {
...
describeEmbeddedPostgres("penclip company import/export e2e", () => {
  ...
  afterAll(async () => {
    ...
    await bestEffortCleanup(tempDb?.cleanup(), 10_000);
    if (tempRoot) {
