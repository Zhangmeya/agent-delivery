...
1. Read config using safe helpers (`asString`, `asNumber`, etc.) from `@penclipai/adapter-utils/server-utils`
...
|--------|--------|---------|
| `runChildProcess(cmd, opts)` | `@penclipai/adapter-utils/server-utils` | Spawn with timeout, grace, streaming |
| `buildPaperclipEnv(agent)` | `@penclipai/adapter-utils/server-utils` | Inject `PAPERCLIP_*` env vars |
| `renderTemplate(tpl, data)` | `@penclipai/adapter-utils/server-utils` | `{{variable}}` substitution |
| `asString(v)` | `@penclipai/adapter-utils` | Safe config value extraction |
| `asNumber(v)` | `@penclipai/adapter-utils` | Safe number extraction |
...
`format-event.ts` — pretty-prints stdout for `penclip run --watch` using `picocolors`.
