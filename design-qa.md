...
- 已修复任务详情顶部全局面包屑的示例任务名称显示：复用现有 `displaySeededName` 只读映射，不修改真实任务数据；任务详情回归测试 44 项通过。
- 其他已改造页面的现有截图早于本批主导航调整，任务详情面包屑修复后也需补捕当前截图；完成全范围视觉证据更新前，final result 暂时保持 blocked。
...
- 修复：任务详情顶部全局面包屑复用示例名称只读显示规则，原始可编辑值、路由和 API 数据保持不变；任务详情回归测试 44 项通过。
- 剩余：任务详情面包屑修复后的当前截图，以及其他页面当前主导航版本截图仍需补捕。
