...
describe("sandbox managed runtime", () => {
  ...
  it("syncs workspace and assets through a provider-neutral sandbox client", async () => {
    ...
    const remoteWorkspaceDownloadTar = path.posix.join(runtimeRootDir, "workspace-download.tar");
    ...
    const removeRemoteWorkspaceContents = async () => {
      ...
      const preserved = new Set([".paperclip-runtime", ".claude"]);
