...
describe("sandbox callback bridge", () => {
  ...
  it("serializes remote response writes so stop does not recreate a late orphaned response", async () => {
    ...
    const handlerDelayMs = process.platform === "win32" ? 2_000 : 250;
    ...
    for (let attempt = 0; attempt < 200 && seenRequestIds.length === 0; attempt += 1) {
      await new Promise((resolve) => setTimeout(resolve, 25));
    }
    ...
    await new Promise((resolve) => setTimeout(resolve, handlerDelayMs + 250));
