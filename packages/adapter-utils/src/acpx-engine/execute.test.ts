...
import type { AdapterRuntimeMcpAccess } from "@penclipai/adapter-utils";
import { DEFAULT_REMOTE_SANDBOX_ADAPTER_TIMEOUT_SEC } from "@penclipai/adapter-utils/execution-target";
import {
  ...
  return {
    execute: async (input: {
      ...
      if (usesPosixShell) {
        ...
        if ((args[0] === "-c" || args[0] === "-lc") && typeof args[1] === "string") {
          args[1] = rewriteWindowsPathsForGitShell(args[1]);
        }
      }
      return await runChildProcess(`acpx-sandbox-run-${counter}`, command, args, {
...
describe("gemini ACP flag selection", () => {
  ...
  async function writeFakeGemini(binDir: string, version: string) {
    ...
    const body = process.platform === "win32"
      ? `@echo off\r\necho ${version}\r\n`
      : `#!/bin/sh\necho "${version}"\n`;
    await fs.writeFile(binPath, body, { mode: 0o755 });
  }
