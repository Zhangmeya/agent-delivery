...
import type { AdapterExecutionContext, AdapterExecutionResult } from "@penclipai/adapter-utils";
import {
...
import { shellQuote } from "@penclipai/adapter-utils/ssh";
import {
  createAcpRuntime,
  createAgentRegistry,
  createRuntimeStore,
...
async function normalizeGeminiAcpCommandShell(commandShell: string, env: NodeJS.ProcessEnv): Promise<string> {
  ...
  try {
    ...
    const result = await runChildProcess(`gemini-version-probe-${randomUUID()}`, tokens[0], ["--version"], {
      cwd: process.cwd(),
      env: probeEnv,
      timeoutSec: Math.max(1, Math.ceil(GEMINI_VERSION_PROBE_TIMEOUT_MS / 1000)),
      graceSec: 1,
      onLog: async () => {},
    });
    if (result.timedOut || result.exitCode !== 0) return commandShell;
    versionParts = parseGeminiVersionParts(result.stdout);
  } catch {
    return commandShell;
  }
...
async function ensureSymlink(target: string, source: string): Promise<void> {
  ...
  if (!existing) {
    ...
    await linkOrCopySharedFile(resolvedSource, target);
    return;
  }
