...
const TRACKED_SKILL_FILES = execFileSync(
  "git",
  ["ls-files", "-z", "--", ...SKILL_FRONTMATTER_ROOTS.map((root) => path.relative(REPO_ROOT, root))],
  { cwd: REPO_ROOT },
)
  .toString("utf8")
  .split("\0")
...
describe("shipped skills catalog", () => {
  ...
  it("keeps repo and catalog skill descriptions within the prompt budget cap", () => {
    ...
    for (const skillFile of TRACKED_SKILL_FILES) {
  ...
  it("exposes a stable manifest header for downstream consumers", () => {
    ...
    expect(catalogManifest.packageName).toBe("@penclipai/skills-catalog");
