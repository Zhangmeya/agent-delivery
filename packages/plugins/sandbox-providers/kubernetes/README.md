# @penclipai/plugin-kubernetes (alpha)
...
```bash
penclip plugin install @penclipai/plugin-kubernetes
```

Or, for local development:

```bash
penclip plugin install --local /path/to/paperclip/packages/plugins/sandbox-providers/kubernetes
```
